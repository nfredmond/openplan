...
module.exports = {
  ...
  async run({ page, baseUrl, expect, campaignShareToken }) {
    expect(
      Boolean(campaignShareToken),
      'No active campaign share token was supplied, so there is no public portal to open and this run proved nothing.',
    );

    await page.goto(`${baseUrl}/engage/${campaignShareToken}`, { waitUntil: 'load' });
    ...
    await expect
      .poll(() => next.isEnabled(), { timeout: 30000 })
      .toBe(true)
      .catch(() => {
        throw new Error(
