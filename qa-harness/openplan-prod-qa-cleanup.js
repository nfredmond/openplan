...
const createdAfter = readOption('--created-after') || process.env.CLEANUP_DATE || outputDate;
const qaPatternSource =
  process.env.OPENPLAN_QA_CLEANUP_PATTERN ||
  'qa|proof|trace|canary|debug|smoke|scenario-compare|managed-run|county-scaffold|layout-audit';
...
if (!/^\d{4}-\d{2}-\d{2}$/.test(createdAfter)) {
  throw new Error(`--created-after must be YYYY-MM-DD, received ${JSON.stringify(createdAfter)}`);
}
...
function createdAtFilter() {
  return `${createdAfter}T00:00:00Z`;
}

function workspaceMatches(workspace) {
  return qaPattern.test(workspace?.name || '') || qaPattern.test(workspace?.slug || '');
}
...
async function main() {
  ...
  const qaWorkspaces = allWorkspaces.filter(workspaceMatches);
  ...
  const [
    ...
    restSelect('reports', 'id,workspace_id,project_id,title,created_at', `&created_at=gte.${createdAtFilter()}&order=created_at.asc`),
    restSelect('engagement_campaigns', 'id,workspace_id,project_id,title,created_at', `&created_at=gte.${createdAtFilter()}&order=created_at.asc`),
  ...
  const projects = (Array.isArray(projectsResult.data) ? projectsResult.data : []).filter(
    (project) => qaPattern.test(project.name || '') || workspaceIds.includes(project.workspace_id)
  );
  const reports = (Array.isArray(reportsResult.data) ? reportsResult.data : []).filter(
    (report) => qaPattern.test(report.title || '') || workspaceIds.includes(report.workspace_id)
  );
  const campaigns = (Array.isArray(campaignsResult.data) ? campaignsResult.data : []).filter(
    (campaign) => qaPattern.test(campaign.title || '') || workspaceIds.includes(campaign.workspace_id)
  );
  ...
  const [reportArtifactsResult, reportRunsResult, reportSectionsResult, engagementCategoriesResult, engagementItemsResult, countyRunArtifactsResult] =
    await Promise.all([
  ...
  summary.deletePlan = deleteSteps
    .filter(([, , values]) => values.length)
    .map(([table, column, values]) => ({ table, column, plannedCount: values.length }));
  summary.authPlan = qaUsers.map((user) => ({ userId: user.id, email: user.email, created_at: user.created_at }));

  if (applyMode) {
    for (const [table, column, values] of deleteSteps) {
      if (!values.length) continue;
      const result = await restDelete(table, inFilter(column, values));
      summary.deleteResults.push({
        table,
        column,
        countPlanned: values.length,
        ok: result.ok,
        status: result.status,
        deletedCount: Array.isArray(result.data) ? result.data.length : null,
    ...
    const verifyWorkspaces = await restSelect(
      'workspaces',
      'id,name,slug,created_at',
      `&created_at=gte.${createdAtFilter()}&order=created_at.asc`
    );
    const remainingWorkspaces = (Array.isArray(verifyWorkspaces.data) ? verifyWorkspaces.data : []).filter(workspaceMatches);
    const remainingUsers = (await listAdminUsers()).filter((user) => {
      const email = user.email || '';
      const createdAt = user.created_at || '';
      return qaPattern.test(email) && createdAt >= `${createdAfter}T00:00:00`;
    });
    ...
    summary.verification = {
      note: 'Dry-run only. No production rows were deleted and no auth users were removed.',
      remainingWorkspaceCount: workspaceIds.length,
      remainingUserCount: qaUsers.length,
    };
  }
  ...
  const lines = [
    ...
    ...(summary.deletePlan.length
      ? summary.deletePlan.map((item) => `- ${item.table}: planned=${item.plannedCount}`)
      : ['- No matching relational rows found.']),
    '',
    '## Auth plan',
    ...(summary.authPlan.length
      ? summary.authPlan.map((item) => `- ${item.email}`)
      : ['- No matching auth users found.']),
    '',
    ...(applyMode
      ? [
          '## Delete results',
          ...(summary.deleteResults.length
            ? summary.deleteResults.map(
                (item) => `- ${item.table}: status=${item.status} ok=${item.ok} planned=${item.countPlanned} deleted=${item.deletedCount ?? 'n/a'}`
              )
    ...
    '## Verification',
    ...(summary.verification.note ? [`- ${summary.verification.note}`] : []),
