...
OpenPlan states its limits as plainly as its strengths — several are enforced by tests. Modeling outputs are screening-grade with caveats attached, not calibrated or validated forecasting. LAPM support is delivery tracking and an invoice register, not exact Caltrans exhibit/E-76 form generation. Aerial operations cover mission planning, source-image custody, orthomosaic processing, verification, and planner-selected report evidence; they do not certify survey accuracy or publish imagery automatically. It should not be described as a finished autonomous municipal SaaS or a substitute for qualified planning review.
