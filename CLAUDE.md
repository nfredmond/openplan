...
- **Phase 3 Slice D - Project markers on cartographic backdrop (2026-04-21).** Second data-driven layer; proves the helper + feature-ref + highlight pattern generalizes beyond AOIs. New migration `20260421000065_projects_location.sql` adds nullable `latitude`/`longitude` NUMERIC columns to `projects` with range check constraints (display-only markers, never spatial queries, so lat/lng beats PostGIS geography here). New `GET /api/map-features/projects` route (~120 LOC) mirrors aerial-missions: auth-gated, workspace-scoped, `.not("latitude", "is", null).not("longitude", "is", null).limit(500)`, defensive `coerceLat`/`coerceLng` handling PostgREST's number-or-string NUMERIC shape, structured audit (`project_markers_loaded` / `_query_failed` / `_unhandled_error`), emits Point features with `{kind: "project", projectId, name, status, deliveryPhase, planType}`. New `project-feature-to-selection.ts` helper (69 LOC) mirrors the mission helper's shape with the optional `sourceId -> featureRef` round-trip. `cartographic-map-backdrop.tsx` gains a second fetch effect, a `#1f6b5e` circle layer (radius 6->9, stroke 1.5->2.5 on feature-state `selected`), a `layers.projects` visibility toggle, click/hover handlers dispatching through the new helper, and a generalized highlight effect that iterates `KNOWN_SOURCES = [AOI_SOURCE_ID, PROJECTS_SOURCE_ID]` (closes the Slice C flagged gap). `scripts/seed-nctc-demo.ts` gains `DEMO_PROJECT_LATITUDE = 39.239137` / `_LONGITUDE = -121.033982` (the Grass Valley anchor) threaded into both `buildSeedRecords` and the live upsert. 12 helper tests + 4 route tests + 1 seed pin. Tests 881/187 -> 898/189; `pnpm qa:gate` clean. **Close-out (same day):** migration `20260421000065` applied to prod via `supabase db push` after a surgical `migration repair` pass reconciled the 16-on-16 local/remote drift left over from the 2026-04-19 MCP batch (registry now fully aligned); demo re-seed landed with `latitude=39.239137` / `longitude=-121.033982` on the NCTC demo project; service-role `SELECT` confirmed both. Browser smoke test remains user-owned (checklist in `~/.claude/plans/joyful-gliding-quilt.md` → Step 3; needs demo-owner browser session). Slice E (corridor layer) is unblocked. Proof: `docs/ops/2026-04-21-phase-3-slice-d-project-markers-proof.md`.
- **Phase 3 Slice B - Click-to-select mission AOIs from the backdrop (2026-04-21).** Closes the reverse direction of the Phase 2 feedback loop (list -> inspector + now map -> inspector). New pure helper `src/lib/cartographic/mission-feature-to-selection.ts` (60 LOC, type guard + transform): maps `{kind: "aerial_mission", missionId, projectId, title, status, missionType}` properties to `CartographicInspectorSelection` with `kind: "mission"`, `"Aerial mission"` kicker, status/type meta chips, `"Open mission"` primary action, and conditional `"Open project"` secondary when `projectId` is present. `cartographic-map-backdrop.tsx` (264 -> 306 LOC) adds a click-handler effect that registers Mapbox `click` / `mouseenter` / `mouseleave` on `AOI_FILL_LAYER_ID` with a stable `navigateRef` closure (so router identity changes don't re-subscribe). Pointer cursor toggles on polygon hover. Helper injected with `navigate` (not a hard `next/navigation` dep) so it's router-free unit-testable. 11 new tests in `src/test/mission-feature-to-selection.test.ts` (guard true/false branches + transform shape + primary/secondary action navigation + null-projectId and blank-title fallbacks). Deliberately no Mapbox+JSDOM integration test - the three-line handler shim is covered by the helper. A11y limit: map clicks are mouse/pointer only; keyboard path is still list-row hover/focus from Phase 2's selection-link sweep. Tests 868/186 -> 879/187; `pnpm qa:gate` clean. Proof: `docs/ops/2026-04-21-phase-3-slice-b-click-to-select-proof.md`.
- **Phase 3 Slice C - Feature-state highlight on selection (2026-04-21).** The clicked AOI polygon now visually lifts on the map, mirroring the inspector dock's selection state. `CartographicInspectorSelection` gains an optional `featureRef: { sourceId, featureId }` for round-trip. The mission helper gains an optional `sourceId` option that populates `featureRef`; unset = no highlight (layer-agnostic). Backdrop paint expressions on both AOI layers (fill + outline) switched to `["case", ["boolean", ["feature-state", "selected"], false], <lifted>, <base>]` — fill-opacity `0.18 -> 0.36`, line-width `1.75 -> 2.75`, color unchanged. New highlight `useEffect` watches `[selection, ready, aois]`, wraps apply in `map.isStyleLoaded()` / `map.once("style.load", apply)` (setStyle wipes feature-state along with sources), clears the whole source state before writing the single current selection (volume is =<500 features, tracking previous ids isn't worth it), and silently swallows `setFeatureState` throws when the feature id isn't loaded yet (rare race during initial fetch). 2 new helper tests (sourceId absent -> featureRef undefined; sourceId present -> featureRef shape). No Mapbox+JSDOM integration test; paint-expression shape is validated at layer-add time by Mapbox itself during `pnpm build`. Tests 879/187 -> 881/187; `pnpm qa:gate` clean. Proof: `docs/ops/2026-04-21-phase-3-slice-c-feature-state-highlight-proof.md`.
- **Phase 3 Slice E - Corridor layer on cartographic backdrop (2026-04-21).** Third data-driven layer; completes pattern-generalization across all three Mapbox primitive geometries (Polygon / Point / LineString). New migration `20260421000066_project_corridors.sql` adds a display-only workspace-scoped `project_corridors` table with jsonb GeoJSON LineStrings (not PostGIS geography — lines are never spatial-query subjects on the backdrop), `corridor_type` CHECK (highway/arterial/transit/bike/trail/custom), nullable `los_grade` A-F, full workspace-member RLS. **Distinct from `network_corridors`** — modeling-side corridors live under `network_packages` and require the full model-package chain. New `isCorridorLineGeoJson` validator (>= 2 positions, finite lng/lat in WGS84 range) guards writes + API reads. New `GET /api/map-features/corridors` route (~95 LOC) mirrors projects: auth-gated, workspace-scoped, `.limit(500)`, defensive geometry filter via the validator, structured audit (`project_corridors_loaded` / `_query_failed` / `_unhandled_error`), emits LineString features with `{kind: "corridor", corridorId, projectId, name, corridorType, losGrade}`. New `corridor-feature-to-selection.ts` helper mirrors the project helper shape (kind `"corridor"`, kicker `"Corridor"`, avatar `"C"`, meta rows for type + optional LOS, primary action to `/projects/:projectId` only when `projectId` is non-null). `CartographicInspectorSelection["kind"]` union extended to include `"corridor"`. Backdrop gains a third fetch + paint effect with an LOS-driven `match` color ramp (A/B calm blue-slate `#4a7a9e` → C/D amber `#c8962f` → E burnt orange `#b45239` → F congestion red `#8a2e24`; no-LOS fallthrough to base), `line-width 3 → 5` on feature-state `selected`, `line-cap/join: round`. `KNOWN_SOURCES` tuple extended to `[AOI, PROJECTS, CORRIDORS]` (no architectural change, Slice D dispatch-table scales as expected). Seed ships 2 hand-authored NCTC corridors: SR-49 through Grass Valley (7 positions, LOS D) + Empire St to Empire Mine (5 positions, LOS C). 15 helper tests + 4 route tests + 3 seed-geometry tests + 8 validator tests (validator shipped earlier in `980ee59`). Tests 906/190 -> 928/193; `pnpm qa:gate` clean (lint, 0 advisories, 63 routes). **Close-out (same day):** migration `20260421000066` applied to prod `aggphdqkanxsfzzoxlbk` via `pnpm supabase db push` (registry aligned cleanly; 4 policy-drop NOTICEs are the expected idempotent no-op). `pnpm seed:nctc -- --env-file .env.production.local` landed both corridors — service-role `SELECT` confirms `Empire St to Empire Mine` (5 positions, LOS C) + `SR-49 through Grass Valley` (7 positions, LOS D) under the demo workspace. Combined Slice D + Slice E browser smoke test remains user-owned (demo-owner session needed). Proof: `docs/ops/2026-04-21-phase-3-slice-e-corridor-layer-proof.md`.
- **Phase 3 Slice F - Layers panel live counts (2026-04-22).** Closes the Slice A TODO on the chip placeholders. New `GET /api/map-features/counts` route (86 LOC) returns `{projects, aerial, corridors}` as workspace-scoped `count: "exact", head: true` queries dispatched in parallel via `Promise.all` (no row payload transfer, ~60-byte JSON response). Filters mirror the layer-fetch routes: projects require non-null lat/lng, aerial requires non-null `aoi_geojson`, corridors count all rows. Per-layer error posture: if one of the three COUNT queries fails, the other two still return their value and the failing one comes back as `null` — panel hides that one chip, doesn't break the whole panel. Structured audit: `map_feature_counts_loaded` / `_partial_failure` / `_unhandled_error`. `cartographic-layers-panel.tsx` (49 → 89 LOC) drops the NCTC-demo placeholders (`14` / `2` / `6` / `3.8k` / `1`), fetches on mount with `AbortController` cancel on unmount, renders chips only for the three data-driven layer keys (the other five stay chip-less honestly until their data sources land). `formatChip` uses `Intl.NumberFormat("en-US", { notation: "compact", maximumFractionDigits: 1 })` for ≥1000; zero renders as `"0"` to distinguish "no rows yet" from "no data source wired". 5 route tests (401 anon / zero-on-no-membership / happy-path all three / partial-failure posture / null-coerce-to-0) + 6 panel tests (pre-fetch empty / live chips / zero-chips / null-hides-one / 1000+ compact / fetch-failure hides all). Tests 928/193 → 939/195; `pnpm qa:gate` clean (lint, 0 advisories, 64 routes). No migration, no seed change — pure additions through existing surface. Proof: `docs/ops/2026-04-22-phase-3-slice-f-layers-panel-live-counts-proof.md`.
- **Phase 3 Slice G - Pan/fit-to-selection + background-click-to-clear (2026-04-22).** Two bundled UX polish items on the backdrop click surface. New pure helper `src/lib/cartographic/geometry-bbox.ts` (78 LOC) exports `fitInstructionFromGeometry(geometry)` returning `{kind: "center", center}` for Point, `{kind: "bbox", bbox}` for LineString / Polygon (outer ring only, interior holes ignored), `null` for MultiPolygon / GeometryCollection / malformed / NaN-Infinity. Defensive: single malformed position in a LineString / outer ring is skipped rather than thrown. `cartographic-map-backdrop.tsx` (636 → 706 LOC) wires fit into the three existing layer-scoped click handlers via a shared local `fitToFeatureGeometry` — Point → `easeTo({zoom: 14, duration: 400})`, bbox → `fitBounds(bbox, {padding: 64, maxZoom: 15, duration: 400})`. **Jitter concern solved by construction:** fit is attached to the click handler, not the `selection` state, so list-row hover that `setSelection`s without a map click never triggers a map movement. New map-level background-click handler uses `queryRenderedFeatures` filtered through `map.getLayer()`-guarded `FEATURE_LAYERS = [AOI_FILL, PROJECTS_CIRCLE, CORRIDORS_LINE]` — zero hits → `clearSelection()`; any hits → layer-scoped handler already ran, no-op. Order-of-operations guarantee: Mapbox fires layer clicks before map clicks. 6 new helper tests in `src/test/geometry-bbox.test.ts` (Point / LineString / Polygon outer-ring / unsupported types / malformed inputs / malformed-position-skip). No Mapbox+JSDOM integration test — Mapbox methods aren't available in JSDOM and the handler itself is a 3-line dispatcher over the pure helper; browser smoke covers the integration surface. Tests 939/195 → 945/196; `pnpm qa:gate` clean (lint, 0 advisories, 64 routes). Pure client-side — no migration, no seed, no new routes. Combined Slice D + E + G browser smoke test remains user-owned. Proof: `docs/ops/2026-04-22-phase-3-slice-g-fit-and-background-click-proof.md`.
