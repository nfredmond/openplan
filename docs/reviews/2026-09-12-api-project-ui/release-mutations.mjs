...
const path=join(app,'src/test/migrations/release-ordering.test.ts'),source=readFileSync(path,'utf8'),scratch=mkdtempSync(join(tmpdir(),'openplan-v055-release-mutations-')),results=[];
...
try{
 for(const [name,before,after,expected] of [
  ...
  ['wrong-migration-count','migrationsAtRelease: 318','migrationsAtRelease: 319','no migration has been inserted at or below a shipped high-water mark'],
  ['missing-migration','lastMigration: "20261012000002_assistant_api_turns.sql"','lastMigration: "20261012000002_missing_api_turns.sql"',"every release's recorded last migration exists on disk"],
 ]){
  ...
  const matched=expected===null?run.status===0&&report.numPassedTests>1:run.status!==0&&failures.some(t=>t.name.includes(expected)&&t.message.includes('AssertionError'));
