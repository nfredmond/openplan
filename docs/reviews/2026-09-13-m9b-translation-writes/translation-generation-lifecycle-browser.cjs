...
const prefix=path.join('/home/nathaniel/.local/state/openplan/response-write-probe-20260913','translation-generation-lifecycle-browser-'+Date.now());fs.mkdirSync(prefix,{mode:0o700});
...
async function journey(browser,width){
 ...
 try{
  ...
  const campaignTitle=`SYNTHETIC lifecycle browser ${width} ${Date.now()}`;await dialog.getByLabel('Title',{exact:true}).fill(campaignTitle);await click(page,dialog.getByRole('button',{name:'Next',exact:true}));await click(page,dialog.getByRole('button',{name:'Create campaign',exact:true}));
  ...
  const readMatcher=url=>url.pathname===generationPath&&url.searchParams.has('requestId');
  await page.route(readMatcher,async route=>{
   heldRequest=route.request();
   try{heldResponse=await route.fetch();expect(heldResponse.status()).toBe(200);readReady();await release;await route.fulfill({response:heldResponse});routeDisposition='response delivered or discarded by cancelled browser request'}
   catch(error){routeDisposition=error.message;readReady();await route.abort('failed').catch(()=>{})}
   finally{readFinished()}
  });
  await click(page,row.getByRole('button',{name:'Generate translation for review',exact:true}));
  await ready;expect(heldResponse?.status()).toBe(200);
  ...
  save(width+'-original-pending.json',local);requestId=local.value.intent.requestId;expect(heldRequest.url()).toContain(requestId);checkpoint('actual queue response retained while verified read is held');
  ...
  await click(page,panel.getByRole('button',{name:/Español.*Spanish/}));await expect(panel.getByRole('button',{name:'Retry same generation request',exact:true})).toBeEnabled();await expect(input).toHaveValue(baseline);
  expect(await page.evaluate(key=>localStorage.getItem(key),local.key)).toBe(local.raw);
  const retainedDownload=handled(page.waitForEvent('download'));await click(page,panel.getByRole('button',{name:'Download generation request',exact:true}));const downloadFile=path.join(prefix,width+'-retained-request.json');await(await retainedDownload).saveAs(downloadFile);expect(JSON.parse(fs.readFileSync(downloadFile))).toEqual(local.value);
  const requestPanel=panel.getByRole('region',{name:'Retained generation request',exact:true});await requestPanel.scrollIntoViewIfNeeded();await page.screenshot({path:path.join(prefix,width+'-recovered-after-sign-in.png')});
  await click(page,panel.getByRole('button',{name:'Review request resolution',exact:true}));await click(page,panel.getByRole('region',{name:'Confirm generation resolution',exact:true}).getByRole('button',{name:'Confirm resolution and preserve copies',exact:true}));await expect(panel.getByText(/Resolution confirmed and copies archived/)).toBeVisible();
  ...
  expect(sql(`select count(*) from engagement_translation_generation_fields where request_id='${requestId}' and state='cancelled'`)).toBe('1');
