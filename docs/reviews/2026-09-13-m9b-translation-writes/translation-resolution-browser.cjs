...
const prefix=path.join('/home/nathaniel/.local/state/openplan/response-write-probe-20260913','translation-resolution-browser-'+Date.now());fs.mkdirSync(prefix,{mode:0o700});
...
async function journey(browser,width){
 ...
 try{
  ...
  await dialog.getByLabel('Title',{exact:true}).fill(`SYNTHETIC resolution browser ${width} ${Date.now()}`);await click(page,dialog.getByRole('button',{name:'Next',exact:true}));await click(page,dialog.getByRole('button',{name:'Create campaign',exact:true}));
  ...
  await page.route('**'+generationPath,async route=>{if(route.request().method()==='POST'){generationIntercepts++;await route.abort('failed')}else await route.continue()});
  ...
  await click(page,panel.getByRole('button',{name:'Retry same resolution',exact:true}));await expect(panel.getByText(/Resolution confirmed and copies archived/)).toBeVisible();expect(sent).toHaveLength(3);expect(sent[1]).toEqual(sent[0]);expect(sent[2]).toEqual(retained.intents[1]);
  ...
  for(const [index,intent] of retained.intents.entries()){
   ...
   expect(archive.receipts[index]).toEqual(receipt);expect(sha(receipt.payloadText)).toBe(receipt.payloadSha256);expect(sha(receipt.resultText)).toBe(receipt.resultSha256);expect(JSON.parse(receipt.resultText).requestExisted).toBe(false);
  }
  ...
  const finalHistory=await page.evaluate(async url=>{const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw Error('Final history unavailable');return(await r.json()).history},historyPath);expect(finalHistory).toEqual(history);await expect(input).toHaveValue(baseline);
...
(async()=>{
 ...
 const sources=['src/components/engagement/translation-resolution-panel.tsx','src/components/engagement/translation-generation-panel.tsx','src/lib/engagement/translation-resolution-recovery.ts','src/app/api/engagement/campaigns/[campaignId]/translations/generation/resolutions/route.ts'];const digests=Object.fromEntries(sources.map(file=>[file,sha(fs.readFileSync(path.join(app,file)))]));save('source-sha256.json',digests);
