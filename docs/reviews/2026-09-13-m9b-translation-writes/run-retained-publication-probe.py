...
original=query("SELECT md5(pg_get_functiondef('public.read_engagement_translation_history(uuid)'::regprocedure)),md5(pg_get_functiondef('public.write_engagement_translations(uuid,uuid,text,text,text,jsonb)'::regprocedure)),has_function_privilege('authenticated','public.write_engagement_translations(uuid,uuid,text,text,text,jsonb)','EXECUTE');")
...
after=query("SELECT md5(pg_get_functiondef('public.read_engagement_translation_history(uuid)'::regprocedure)),md5(pg_get_functiondef('public.write_engagement_translations(uuid,uuid,text,text,text,jsonb)'::regprocedure)),has_function_privilege('authenticated','public.write_engagement_translations(uuid,uuid,text,text,text,jsonb)','EXECUTE');")
