...
mutate('omit-archive-readback','helper','if (storage.getItem(archive) !== raw || storage.getItem(key) !== raw)','if (storage.getItem(key) !== raw)','keeps the refused source copy when archive readback fails')
results=[]
try:
 for name,key,body,expected in cases:
  ...
  correct=run.returncode==0 and report['numPassedTests']==32 if expected is None else run.returncode!=0 and any(expected in f for f in failed)
 ...
 (review/'generation-recovery-controls.json').write_text(json.dumps({'sourceSha256':{k:hashlib.sha256(v.encode()).hexdigest() for k,v in original.items()},'testSha256':hashlib.sha256(test.read_bytes()).hexdigest(),'privateEvidence':str(private),'results':results,'limits':'React and storage failures with synthetic records and mocked HTTP. Tests protect retention and refusal behavior; they do not provide a new way to resolve an unreadable request with no retrievable server record, prove real downloads, or test lifecycle cancellation or public generation.'},indent=2)+'\n')
