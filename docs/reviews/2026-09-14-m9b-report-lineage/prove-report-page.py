...
originals = {p: p.read_text() for p in [page, route]}
page_source = originals[page]
route_block = '''  const specializedReport = await loadSpecializedReportPage(supabase, report);
...
cases = [
    ...
    ('null-project-query', page, '!campaign.error && campaign.data?.project_id', '!campaign.error && campaign.data', 'uses the saved job before'),
    ('campaign-failure-hidden', page, '{campaign.error ? <p role="alert">', '{false ? <p role="alert">', 'unavailable campaign context; failed read=true'),
...
try:
    for label, path, old, new, failure in cases:
        ...
        with (evidence / f'{label}.log').open('w') as log:
            result = subprocess.run(['npm', 'exec', '--', 'vitest', 'run', 'src/test/engagement-review-report-page.test.tsx', '--reporter=json', f'--outputFile={output}'], cwd=app, stdout=log, stderr=subprocess.STDOUT)
        report = json.loads(output.read_text())
        failed = [a['fullName'] for t in report['testResults'] for a in t['assertionResults'] if a['status'] == 'failed']
        passed = report['numPassedTests']
        expected = (result.returncode == 0 and passed == 11 and not failed) if failure is None else (result.returncode != 0 and any(failure in name for name in failed))
