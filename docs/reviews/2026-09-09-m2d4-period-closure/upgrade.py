...
fixture=text[text.index('DO $test$ DECLARE'):text.index('   ${body}')].replace('${extraSource}','').replace('${progress}','Review remains').replace('${paymentDate}','2026-08-01')+"\n PERFORM public.work_program_closeout_command(p,o,close_command); PERFORM public.work_program_closeout_command(p,o,close_command-'assessment'||jsonb_build_object('kind','approve','requestId',gen_random_uuid(),'expectedVersion',1,'note','Synthetic approved reconciliation before upgrade')); INSERT INTO upgrade_ids VALUES(p); END $test$;"
base=(root/'supabase/migrations/20261003000001_work_program_reimbursement.sql').read_text();predecessor=(root/'supabase/migrations/20261004000001_work_program_closeout_reconciliation.sql').read_text();migration=(root/'supabase/migrations/20261005000001_work_program_period_closure.sql').read_text()
...
tables=['work_program_closeout_records','work_program_reporting_periods','programs','program_work_program_revisions','program_work_program_events','work_program_actual_versions','work_program_actual_allocations','work_program_period_reports','work_program_reimbursement_claims','work_program_reimbursement_sources','work_program_reimbursement_events']
...
(out/'upgrade-results.json').write_text(json.dumps({'container':'supabase_db_m11-contract-verification-upgrade','transaction':'rolled back; original stack retained at 306 migrations','baseline':'v0.47.0 plus merged reconciliation schema and approved synthetic reconciliation applied inside transaction','newMigration':'20261005000001_work_program_period_closure.sql','migrationSha256':hashlib.sha256(migration.encode()).hexdigest(),'tables':tables,'results':results},indent=2)+'\n')
