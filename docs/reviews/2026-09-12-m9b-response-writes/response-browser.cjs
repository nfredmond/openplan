...
(async()=>{
 ...
 const errors=[],requests=[],networkFailures=[];let campaignId,worker,workerExit,original,committedStatus;
 ...
 try{
  ...
  await tabs.getByRole('link',{name:'Setup',exact:true}).click();await panel.getByLabel('Reason for this change',{exact:true}).fill('SYNTHETIC reviewed correction');await panel.getByRole('button',{name:'Edit',exact:true}).click();await expect(panel.getByRole('button',{name:'Save',exact:true})).toBeVisible();const editRow=panel.locator('.module-record-row').filter({has:page.getByRole('button',{name:'Save',exact:true})});const editWords=editRow.getByRole('textbox',{name:/^we did$/i});await expect(editWords).toHaveValue('SYNTHETIC original response');await editWords.fill('SYNTHETIC corrected response');await expect(editWords).toHaveValue('SYNTHETIC corrected response');let releaseCorrection,correctionPayload;const correctionGate=new Promise(resolve=>{releaseCorrection=resolve});
  ...
  const finalHistory=await history();expect(finalHistory).toHaveLength(3);expect(finalHistory[0].record_sha256).toBe(initialHistory[0].record_sha256);expect(finalHistory[0].record).toEqual(initialHistory[0].record);await retained.scrollIntoViewIfNeeded();await page.screenshot({path:prefix+'-history.png'});const publication=retained.getByTestId('closeloop-broadcast-notice');await publication.getByRole('button',{name:'Refresh email status',exact:true}).focus();await page.keyboard.press('Enter');await expect(publication.getByText('Preparation found no confirmed, active subscriptions for this campaign. No messages were prepared for this update.',{exact:true})).toBeVisible();await publication.scrollIntoViewIfNeeded();await page.screenshot({path:prefix+'-history-publication.png'});
