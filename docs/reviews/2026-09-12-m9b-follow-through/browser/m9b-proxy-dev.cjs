...
const proxy=http.createServer(async(req,res)=>{
 ...
 if(fail&&req.method==='GET'&&parsed.pathname==='/rest/v1/engagement_closeloop_entries'&&parsed.searchParams.get('campaign_id')===`eq.${campaignId}`){
 ...
 upstream.on('error',()=>{if(!res.headersSent)res.writeHead(502);res.end('Local backend unavailable')});req.pipe(upstream);
});
proxy.on('error',e=>{console.error(e.message);process.exit(1)});
proxy.listen(3218,'127.0.0.1',()=>{
 const fd=fs.openSync('/home/nathaniel/.local/state/openplan/api-provider-research-2026-09-12/m9b-dev.log','w');
