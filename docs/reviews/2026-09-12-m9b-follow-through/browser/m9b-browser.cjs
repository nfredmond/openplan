...
const account=JSON.parse(fs.readFileSync(`${root}/api-settings-account.json`)),base='http://127.0.0.1:3255',width=Number(process.env.WIDTH||1440),title=`SYNTHETIC response recovery ${width} ${Date.now()}`;
...
(async()=>{
 ...
 try{
  ...
  await retry.scrollIntoViewIfNeeded();await page.screenshot({path:`${root}/m9b-${width}-unavailable.png`});
  ...
  const retained=await (await context.request.get(api)).json();expect(retained).toEqual(original);await panel.getByText('SYNTHETIC retained crossing response',{exact:true}).scrollIntoViewIfNeeded();await page.screenshot({path:`${root}/m9b-${width}-recovered.png`});fs.writeFileSync(`${root}/m9b-${width}-aria.txt`,await panel.ariaSnapshot());
