...
const actuals=await service.from('work_program_actual_versions').select('id,entry_id').eq('program_id',fixtures[0].programId);if(actuals.error)throw Error('Actual history unavailable');if(actuals.data.length!==4 || new Set(actuals.data.map(a=>a.entry_id)).size!==4)throw Error('Carryover created or changed physical costs');
...
await writeFile('/home/nathaniel/.local/state/openplan/m2d4-refund-matching-evidence-2026-09-09/access-results.json',JSON.stringify({results,originalApprovalUnchanged:true,originalPeriodClosureUnchanged:true,physicalActualRows:4,physicalEntries:4,privateCaching:true,baselineHash:current.content.source.report.snapshot.baseline.content_sha256,blindCategory:'Synthetic browser identities; actual agency authority and usefulness unproved.'},null,2)+'\n');
console.log('Owner access succeeds; anonymous, member and foreign workspace reads/writes denied; original approval and four physical entries preserved.');
