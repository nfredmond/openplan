...
(async () => {
 ...
 try {
  ...
  await sources.getByRole('heading', { name: 'Retained synthesis sources', exact: true }).evaluate(element => element.scrollIntoView({ block: 'start' }));
  ...
  let omitCharset = false;
  await layout.route(base + '/__synthetic_legacy_layout', route => route.fulfill({ contentType: 'text/html', body: `<!doctype html><html class="dark"><head>${omitCharset ? '' : '<meta charset="utf-8">'}<base href="${base}/">${styles}</head><body><main style="max-width:1100px;margin:auto;padding:16px"><p>Explicit synthetic historical-format layout fixture</p><article class="module-section-surface">${fixture}</article></main></body></html>` }));
  ...
  const retainedLabel = layout.getByText('Neutral · 2 comments', { exact: false });
  await expect(retainedLabel).toBeVisible();
  await layout.evaluate(() => document.head.append(document.createComment('harmless charset control')));
  await expect(retainedLabel).toBeVisible();
  omitCharset = true; await layout.reload();
  await expect(retainedLabel).toHaveCount(0);
  omitCharset = false; await layout.reload(); await expect(retainedLabel).toBeVisible();
  observed.fixtureEncoding = { baseline: 'UTF-8', harmless: 'survives', omittedCharset: 'detected', restored: true };
