...
cases['authority_field_scope']=''
for kind in ['submit','comment','resolve_comment','return','approve','start_amendment']:
 for field,value in [('authority',"'Synthetic hidden authority'"),('scope',"'Synthetic hidden scope'"),('evidenceDate','current_date')]:
  cases['authority_field_scope']+=deny('PERFORM '+cmd(kind,extra=f"jsonb_build_object('{field}',{value})"),'22023',f'HIDDEN_{kind}_{field}')
cases['authority_field_scope']+='PERFORM '+cmd('comment')+';'
legacy_source=(root.parent/'captured/20260909000001_work_program_review.sql').read_text()
legacy_rpc=definition('record_work_program_event',legacy_source)
cases['legacy_retry']='EXECUTE $install$'+legacy_rpc+'$install$;saved:=pg_temp.command(\'comment\',p,r.id)||jsonb_build_object(\'authority\',\'Synthetic legacy hidden field\');SELECT * INTO e FROM public.record_work_program_event(p,a,saved);EXECUTE $install$'+rpc+'$install$;'+check("(SELECT id=e.id FROM public.record_work_program_event(p,a,saved))",'LEGACY_RETRY_CHANGED')+check("(SELECT count(*)=1 FROM public.program_work_program_events WHERE program_id=p)",'LEGACY_RETRY_DUPLICATE')+deny("PERFORM public.record_work_program_event(p,a,saved||'{\"note\":\"changed legacy retry\"}')",'PT409','LEGACY_CHANGED_RETRY')
...
ok,err=run('authority_noop_survivor',cases['authority_field_scope'],rpc.replace('Authority fields require an authority record','Authority fields belong to an authority record'));record('authority_noop_survivor',ok,err)
assert ok,'Authority harmless mutation could not survive'
for name,body in cases.items():
 ok,err=run('baseline_'+name,body);record('baseline_'+name,ok,err)
mutations=[
 ('authority_field_scope',rpc,"IF k NOT IN ('adoption','external_acceptance','spending_authorization','withdraw_authority') AND (coalesce(p_command->>'authority','')<>'' OR coalesce(p_command->>'scope','')<>'' OR p_command->>'evidenceDate' IS NOT NULL)",'IF false','ASSERT_HIDDEN_comment_authority'),
...
for name,definition_,old,new,expected in mutations:
 ...
 body=cases[name] if name!='authority_field_scope' else deny('PERFORM '+cmd('comment',extra="jsonb_build_object('authority','Synthetic hidden authority')"),'22023','HIDDEN_comment_authority')
 ok,err=run('mutant_'+name,body,definition_.replace(old,new))
 record('mutant_'+name,not ok and expected in err,err)
authority_line=next(line for line in rpc.splitlines() if "Authority fields require an authority record" in line)
legacy_reordered=rpc.replace(authority_line+'\n','').replace(" SELECT * INTO e FROM public.program_work_program_events WHERE program_id=p_program_id AND request_id=",authority_line+"\n SELECT * INTO e FROM public.program_work_program_events WHERE program_id=p_program_id AND request_id=")
assert legacy_reordered!=rpc
ok,err=run('mutant_legacy_retry_order',cases['legacy_retry'].replace(rpc,legacy_reordered))
record('mutant_legacy_retry_order',not ok and 'Authority fields require an authority record' in err,err)
