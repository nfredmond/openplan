...
- and its `network_source: "OpenStreetMap"` field is correct for the assignment-network side of the accepted run.
- The preservation requirement is to document the mixed-source proof composition clearly, not to rewrite the evidence packet into a false single-source story.
...
1. Make the mixed-source proof composition explicit in the docs: OSM assignment network + TIGER/Line package-generation lineage.
...
1. Make the mixed-source proof composition explicit in the docs.
2. Create `modeling/placer-proof-checkpoint`.
3. Commit scripts + minimal package inputs + docs + `run_output/evidence_packet.json`.
4. Rerun once from that branch.
5. If rerun matches the current safe claim, keep the evidence packet as the lightweight assignment-run descriptor.
