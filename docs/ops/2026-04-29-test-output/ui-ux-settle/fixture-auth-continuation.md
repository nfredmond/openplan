...
- Current remaining proof-pack gaps are:
  - reports are fixture-ready in the NCTC seed as `/reports/d0000001-0000-4000-8000-000000000019`, pending a fresh local read-only capture.
  - `fixture_required`: `/scenarios`, `/scenarios/<local-scenario-set-id>`, `/grants` at desktop and mobile.
...
| Reports | `/reports`, `/reports/d0000001-0000-4000-8000-000000000019` | Same local workspace; updated NCTC local seed run against local Supabase; no production Supabase. | Rerun the local NCTC seed, then run the capture harness for `reports-index` and `report-detail`. The seed provides a deterministic RTP board-packet record, sections, and HTML artifact; do not generate or mutate reports during the capture pass. | Registry shows `NCTC 2045 RTP settle board packet`; detail shows RTP packet posture, sections, generation history, and the latest HTML packet preview. Desktop/mobile rows become `captured`. |
