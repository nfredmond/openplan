...
4. For unfinished work, prepare and adopt the successor cycle through Programming Cycles first. Select its exact baseline, work element and a carryover fund, then the source fund, amount and authority evidence. The successor may overlap the old cycle but must start later and use the same currency/workspace. Use **Add carryover allocation to work** for each additional source fund or successor destination. Several old work elements may select the same successor. Each allocation keeps its exact baseline, element and funding identities. Fund ceilings are checked across source cycles and successor amendments; they do not calculate available cash or interpret funding conditions.
...
The unpaid claim and outstanding obligations remain with the old cycle. Carryover links work and funding; it creates no second expense. The separate period decision below protects closed accounting dates. Split and merged work can use multiple funding allocations. The JSON is retained evidence, not a general import/restore command. Agency-form compatibility, actual authority and practicing-finance usefulness require separate acceptance.
...
Apply `20261005000001_work_program_period_closure.sql` after the reconciliation migration. The workflow remains an internal accounting-period control. Human finance acceptance, bank-verified disbursements and independent reconstruction/restore remain open. See the [period closure evidence](../reviews/2026-09-09-m2d4-period-closure/VERIFICATION.md) for current verification status.
...
## Multiple carryover allocations

Apply `20261007000001_work_program_multi_carryover.sql` after refund matching. Earlier single-mapping approvals remain unchanged and downloadable. Editing a mapping converts that work element to allocation rows in the new draft. Empty or unknown amounts are refused for carryover; explicit zero stays zero. Completion and unassessed work cannot retain funding allocations.

Select each allocation's source fund, exact adopted successor baseline, work element and carryover fund, then its amount. Changing the successor baseline clears dependent element/fund selections. Use the shared work-element evidence to explain every allocation's authority and conditions. Save, approve and correct through the existing reconciliation controls. Combined allocations must fit assessed source and successor fund ceilings; those ceilings do not calculate spendable cash.

[Multiple carryover engineering evidence](../reviews/2026-09-09-m2d4-multi-carryover/VERIFICATION.md) retains the implementation and verification status. Independent reconstruction/restore and finance acceptance remain open.
