...
export function buildWorkspaceOperationsSummary({
  ...
  const grantModelingLeadDecisionDetail = firstFundingDecisionProject
    ? `${firstFundingDecisionProject.leadOpportunity?.title ?? "The lead funding decision"} for ${firstFundingDecisionProject.project.name} ${firstFundingDecisionProject.modelingReadiness?.key === "decision-ready" ? "is rising because its modeling evidence looks decision-ready." : firstFundingDecisionProject.modelingReadiness ? `is still waiting for a decision while its modeling evidence reads ${firstFundingDecisionProject.modelingReadiness.label.toLowerCase()}.` : "is still waiting for a decision, with no modeling evidence attached."}${firstFundingDecisionProjectLeadPacketDetail ? ` ${firstFundingDecisionProjectLeadPacketDetail}` : ""}${firstFundingDecisionProjectRecommendation ? ` Recommended next move: ${firstFundingDecisionProjectRecommendation.recommendedNextActionTitle}. ${firstFundingDecisionProjectRecommendation.recommendedNextActionSummary}` : ""}`
    : null;
