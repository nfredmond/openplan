...
export function buildPilotReadinessPacket(statusList: PilotReadinessPacketStatus[], generatedAt = new Date().toISOString()) {
  const lines = [
    ...
    "- Cite source artifacts, not dashboard summaries: each PASS lane must point to the named proof file or generated packet artifact before buyer reliance.",
    "- Re-run or refresh any FAIL, PENDING, or UNKNOWN lane before using this packet for pilot diligence.",
    "- This export is not an autonomous launch certificate; it preserves supervised implementation, human review, and no-autonomous-AI caveats from the release proof packet.",
