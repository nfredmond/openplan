 ...
 *
 * Output was originally byte-identical to the Jinja renders (verified against
 * jinja2 3.1.6 with `trim_blocks=False`, `keep_trailing_newline=True`); the
 * deliberate divergences since are: the platform name ("ClawModeler" reads
 * "OpenPlan"), the opt-in calibrated-basis disclosure, and the "Jurisdiction
 * basis and modeling claim tier" section of the CEQA memo — added because the
 * exported memo is the quotable artifact, and a determination that travels
 * without its jurisdiction basis and claim tier is exactly the overclaim
 * `summarizeVmtDetermination` exists to prevent on-screen. Every caveat,
 * statutory citation, and disclaimer string is otherwise preserved verbatim;
 * these memos are screening-level drafts, not determinations of record, and
 * the wording that says so must not drift.
 */
...
/**
 * Whose law — if anyone's — stands behind an exported CEQA memo.
 *
 * The on-screen panel distinguishes a framework that GOVERNS the workspace
 * (statutes cited as authority) from the absence of one (the same statutes cited
 * only as the source of the method). The memo is the artifact that leaves the
 * tool and gets attached to a staff report, so the same distinction must travel
 * into it — a memo without it reads as authority everywhere.
 */
export type CeqaMemoJurisdictionBasis =
  | {
      kind: "framework";
...
export type RenderCeqaVmtOptions = RenderPlannerPackOptions & {
  /**
   * Omitted → rendered as "none established". The default is deliberately the
   * CONSERVATIVE arm: a caller that forgets to pass the basis produces a memo
   * that under-claims, never one that presents California law as authority over
   * a workspace nobody resolved.
   */
  jurisdictionBasis?: CeqaMemoJurisdictionBasis;
  /** The run's recorded claim tier as the surface read it. Omitted/null →
   * stated as not established, in the same shared wording the screen uses. */
...
export function renderCeqaVmtMarkdown(
  ...
  { runId, engineVersion, calibratedBasis = false, jurisdictionBasis, claimTier }: RenderCeqaVmtOptions
): string {
...
## Jurisdiction basis and modeling claim tier

${
    jurisdictionBasis?.kind === "framework"
      ? `- **Jurisdiction basis:** ${jurisdictionBasis.frameworkName}, ${jurisdictionBasis.jurisdictionLabel}. ${jurisdictionBasis.scopeStatement}`
      : `- **Jurisdiction basis: none established.** ${
          jurisdictionBasis?.reason ??
          "This memo was generated by a surface that does not resolve which jurisdiction's VMT significance rules apply, so OpenPlan cannot say whose determination this would be."
        } The citations above are the source of the screening method, not authority over a project in your area — the determinations in this memo are arithmetic screening results, not significance determinations for your area.`
  }
- **Modeling claim tier:** ${describeClaimTierForDetermination(claimTier)}
