...
export type DeliveryState={assignments?:{taskId:string;staffId:string}[];scheduleVersions:ScheduleVersion[];workUpdates:WorkUpdateVersion[];capacityVersions:CapacityVersion[];outsideReservations:OutsideReservation[];inputHash:string;forecasts:ForecastVersion[]};
export type ForecastWarning={code:string;nodeId:string|null;nodeMask?:string;staffId:string|null;date:string|null;message:string};
// Format 3 shares capacity warnings per person/date. nodeMask is a hexadecimal bit set: bit i identifies nodes[i]; nodeId remains for individual warnings.
// Format 2 sums this assignment's reservations per person/date. The retained input schedule preserves task attribution; format 1 keeps its original repeated rows.
export type ForecastResult={formatVersion:1|2|3;asOf:string;horizonEnd:string;finish:string|null;remainingCost:string|null;actualPlusRemaining:string|null;remainingGrossBilling:string|null;coverageComplete:boolean;warnings:ForecastWarning[];nodes:{id:string;title:string;taskId:string;start:string|null;finish:string|null;originalApprovedFinish:string|null;currentApprovedFinish:string|null;actualStart:string|null;actualFinish:string|null}[];reservations:OutsideReservation[]};
