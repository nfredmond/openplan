...
import { normalizeContractCommand } from "./calculation";
...
export async function saveContractCommand(request: NextRequest, engagementId: string, input: unknown) {
 ...
 try{normalized=await normalizeContractCommand(access.service!,engagementId,access.user!.id,command.data);}catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Contract calculation unavailable"},{status:400});}
