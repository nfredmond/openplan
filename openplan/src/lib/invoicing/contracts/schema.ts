import { closeoutCommandSchemas, type CloseoutState } from "./closeout-schema";
import { responseCommandSchemas, type ResponseState } from "./response-schema";
...
export const contractCommandSchema = z.discriminatedUnion("kind", [
 ...
 ...responseCommandSchemas,
 ...closeoutCommandSchemas,
...
export type ActualVersion = { member_can_correct?: boolean; shared_source_stale?: boolean; id: string; entry_id: string; version: number; command: ContractActual; amount: string | null; hours: string | null; allocations: ContractAllocation[]; time_entry_id: string | null; spend_entry_id: string | null; created_at: string };
...
export type ReceivedInvoice = {source_receipt?:{id:string;filename:string;contentType:string;checksum:string;bytes:number}|null;id:string;invoice_id:string;version:number;state:"submitted"|"returned"|"reviewed"|"approved";content:z.infer<typeof receivedContentSchema>&{total:string;fileId:string};matches?:{entryId:string;versionId:string;amount:string;staffId?:string|null;sourceKey?:string;category?:string;allocations?:ContractAllocation[]}[];review_note:string;created_at:string};
