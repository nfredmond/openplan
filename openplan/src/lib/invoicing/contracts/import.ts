...
export function previewContractCsv(csv:string,filename:string,mapping:ContractImportMapping,defaults:ContractActual,requestId:string,tasks:{id:string;title?:string;deliverableId:string|null}[],staff:{id:string;name:string}[]=[]) {
 ...
 const rows=records.map((record,index)=>{
  ...
  const matchingTasks=values.taskId?tasks.filter(t=>t.id===values.taskId||t.title===values.taskId):[];
  const task=matchingTasks.length===1?matchingTasks[0]:undefined;
  const matchingStaff=values.staffId?staff.filter(s=>s.id===values.staffId||s.name===values.staffId):[];
  const person=matchingStaff.length===1?matchingStaff[0]:undefined;
  const attributionErrors=[...(values.taskId&&!task?["Task must match one approved task name or ID"]:[]),...(values.staffId&&!person?["Staff must match one staff name or ID"]:[])];
  if(person)values.staffId=person.id;
  const candidate={...defaults,...Object.fromEntries(Object.entries(values).filter(([k])=>k!=="taskId")),kind:"actual",status:"draft",expectedVersion:0,requestId:stableId(requestId,index,"request"),entryId:stableId(requestId,index,"entry"),sourceReference:`${filename}; sha256:${hash}; CSV row ${index+1}; ${defaults.sourceReference}`,allocations:values.taskId?[{taskId:task?.id??values.taskId,deliverableId:task?.deliverableId??null,share:10000}]:defaults.allocations};
