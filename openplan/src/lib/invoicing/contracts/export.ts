...
export function contractSnapshotTables(report: ContractSnapshot): { name: string; rows: Cell[][] }[] {
 ...
 if(state.schemaVersion===2){
  tables.push({name:"Master authorization",rows:[["Version","State","Currency","Shared ceiling","Authorized start","Authorized end","Terms","Approval evidence"],...(state.masterTerms??[]).map(t=>[t.version,t.state,t.currency,t.ceiling,t.starts_on,t.ends_on,t.terms,t.approval_evidence])]});
