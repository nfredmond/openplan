 ...
 let normalized:unknown=command;
  ...
  if(command.kind==="settlement"||command.kind==="closeout"){
   ...
   if(command.kind==="settlement"){
    ...
    normalized={...command,_request:command,_inputHash:state.closeout.inputHash,_position:position,_package:{formatVersion:1,state:safeState,position,request:command}};
   }
  }
