...
export type ParticipantAssignment={id:string;title:string;returned_invoices:number;open_for_work:boolean|null};
...
export async function loadParticipantWork(client:SupabaseClient):Promise<ParticipantWork>{
 try{
  const result=await readEveryPage<ParticipantAssignment>((from,to)=>client.from("contract_participant_my_work").select("id,title,returned_invoices,open_for_work").order("id").range(from,to));
