...
export type ActualVersion = { id: string; entry_id: string; version: number; revision_id: string; source_key: string; entry_date: string; kind: ActualCommand["kind"]; staff_id: string | null; time_entry_id?: string | null; spend_entry_id?: string | null; staffName?: string | null; costRate?: { id: string; hourly_cost: string; starts_on: string; ends_on: string; source_reference: string } | null; amount: string | null; hours: string | null; status: ActualCommand["status"]; valuation_basis: string; detail: ActualCommand; allocations: ActualAllocation[]; created_at: string };
...
export type ReportSnapshot = { reimbursement?: import("./reimbursement").ReimbursementSnapshot; schemaVersion: 1; workingPreview?: true; period: ReportingPeriod; baseline: { id: string; revision: number; content_sha256: string; content_json: WorkProgramDraft; source_ids: string[] }; actuals: ActualVersion[]; valuationHistory: ActualVersion[]; reviewNote: string; issueNote?: string };
