...
async function exportDocument(documentId: string) {
  ...
  const document = await service.from("kb_documents").select("id, workspace_id, work_program_revision_id, work_program_export_format, work_program_packet_id, work_program_packet_format, work_program_report_id, work_program_report_format, content_type").eq("id", documentId).single();
...
export async function loadWorkProgramExportIdentity(documentId: string) {
  const service = createServiceRoleClient();
  const { row: document, packet, report } = await exportDocument(documentId);
...
export async function renderWorkProgramExport(documentId: string) {
  const service = createServiceRoleClient();
  const { row, packet, report } = await exportDocument(documentId);
