...
export function reportTables(report: PeriodReport): { name: string; rows: Cell[][] }[] {
 ...
 const tables: { name: string; rows: Cell[][] }[] = [{ name: "Identity", rows: [
  ["Internal management report", s.period.name], ["Version", report.version], ["Issued", report.issued_at], ["Period start", s.period.starts_on], ["Period end", s.period.ends_on], ["Source cutoff", s.period.source_cutoff],
  ["Currency", s.baseline.content_json.currency], ["Baseline revision", s.baseline.revision], ["Baseline ID", s.baseline.id], ["Baseline SHA256", s.baseline.content_sha256], ["Report ID", report.id], ["Snapshot SHA256", report.snapshot_hash], ["Corrects report", report.corrects_report_id ?? "Original issue"],
  ...
  ["Scope", "Internal management only. No funder reimbursement, payroll replacement, spending authority, accepted work or forecast finish date is established."],
 ...
 for (const table of reportTables(report)) {
  ...
  const rows = table.rows.flatMap((row, index) => {
   const parts = row.map((value, column) => {
    ...
    if (index > 0 && column > 0 && typeof v === "string" && /^-?\d+\.\d{2}$/.test(v) && Math.abs(Number(v)) < 1e12) return [Number(v)];
...
export async function renderPeriodReport(report: PeriodReport, format: "pdf" | "xlsx") {
 ...
 const pdf = await renderReportPdf(buildPeriodReportHtml(report), { title: report.snapshot.period.name, generatedAt: report.issued_at, footerLabel: `Internal management v${report.version}` });
