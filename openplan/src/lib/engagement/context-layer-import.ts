 ...
 *
 * NODE-ONLY. This module inflates zip archives with `node:zlib`, so it is
 * imported by the upload route and by tests, never by a client component. The
 * types and the query that client code needs live in `context-layers.ts`.
 *
 ...
 *     specification is not a guess, and `srs_basis` records which of these it
 ...
 * drawn — and it is COUNTED, so the layer's own disclosure reports it as a shape
 * that is in the file and not on the map. See `READABLE_SHAPE_TYPES` for why the
 * obvious modulo shortcut turns that skip into a wrong dot instead.
 */
...
export type ContextLayerImportRefusalReason =
  | "unsupported_format"
  | "unreadable"
  | "empty"
  | "srs_undetermined"
...
export type ContextLayerImport = {
   ...
   * sides have to be in the same unit — which is why the sentence
   * `describeContextLayerCoverage` writes says "shapes" and not "features".
   */
  sourceFeatureCount: number;
  /** Shapes present in the file that are not on the map: unusable coordinates,
   * or a shape type this reader refuses to draw rather than draw wrongly. */
...
function refuse(reason: ContextLayerImportRefusalReason, message: string): ContextLayerImportRefusal {
 ...
 * actually inside.
 */
export function detectContextLayerFormat(filename: string, bytes: Uint8Array): ContextLayerFormat | null {
...
export function importContextLayer(input: ContextLayerImportInput): ContextLayerImportResult {
  if (input.bytes.byteLength === 0) {
    return refuse("empty", "The uploaded file is empty.");
  }

  const format = detectContextLayerFormat(input.filename, input.bytes);
  if (!format) {
    return refuse(
      "unsupported_format",
  ...
  const read =
    format === "geojson"
      ...
      : format === "kml"
        ...
        : format === "kmz"
          ? readKmzUpload(input.bytes)
          : readShapefileUpload(input.bytes);
...
type ReaderResult =
  | {
       ...
       * them out would make the layer's own disclosure say the file ended where
       * they began.
       */
      undrawableCount?: number;
    }
  | ContextLayerImportRefusal;
...
const BASE_KIND: Record<string, ContextLayerGeometryKind> = {
  Point: "Point",
  MultiPoint: "Point",
...
function normalize(
  ...
  featureCap: number | null,
  undrawableCount: number
): ContextLayerImportResult {
  const bounds: Bounds = { west: 180, south: 90, east: -180, north: -90 };
  const features: GeoJSON.Feature[] = [];
  const kinds = new Set<ContextLayerGeometryKind>();
  ...
  for (const candidate of candidates) {
    if (featureCap !== null && features.length >= featureCap) {
      // Everything from here on is counted but not stored, so the layer can say
  ...
  if (candidates.length === 0) {
    return refuse(
      "no_drawable_features",
      `This ${describeFormat(format)} carried no point, line, or area geometry that OpenPlan could read, so there ` +
  ...
  if (features.length === 0) {
    return refuse(
      "no_drawable_features",
      `Every feature in this ${describeFormat(format)} had coordinates outside the valid longitude/latitude range, ` +
...
function describeFormat(format: ContextLayerFormat): string {
  switch (format) {
    case "geojson":
      return "GeoJSON file";
    case "kml":
...
const WGS84: Omit<ContextLayerSrs, "basis"> = { authority: "EPSG", code: "4326", name: "WGS 84" };
...
function readGeoJsonUpload(bytes: Uint8Array): ReaderResult {
  ...
  let basis: ContextLayerSrs["basis"] = "geojson_rfc7946_default";
...
function readKmlUpload(text: string): ReaderResult {
  ...
  // is following the specification, not assuming, and `srs_basis` says which.
...
function readKmzUpload(bytes: Uint8Array): ReaderResult {
  const entries = readZipEntries(bytes);
  if (!entries.ok) return entries;

  const kml = entries.files.find((file) => file.name.toLowerCase().endsWith(".kml"));
  if (!kml) {
    return refuse(
      "unreadable",
...
type ZipResult = { ok: true; files: ZipFile[] } | ContextLayerImportRefusal;
...
function readZipEntryNames(bytes: Uint8Array): string[] {
  const entries = readZipEntries(bytes, { namesOnly: true });
  return entries.ok ? entries.files.map((file) => file.name) : [];
}
 ...
 */
function readZipEntries(bytes: Uint8Array, options: { namesOnly?: boolean } = {}): ZipResult {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    if (options.namesOnly) {
      files.push({ name, bytes: new Uint8Array(0) });
      continue;
    }
    ...
    try {
      // BOUNDED BY WHAT THE ARCHIVE ITSELF DECLARES. An unbounded
      // `inflateRawSync` will expand whatever the compressed stream says to,
      // and deflate reaches roughly a thousand to one on repetitive input — a
      // 200 KB upload that inflates to 200 MB inside this process is a few
      // lines of script to produce. The central directory already states the
      // uncompressed size, so holding the inflate to it costs nothing and turns
      // a lying header into a named refusal instead of an out-of-memory kill.
      // It does NOT bound an archive that honestly declares a huge entry; that
      // is the operator's byte ceiling's job, and unset still means unlimited.
      // `maxOutputLength` must be at least 1, and a legal zero-byte entry would
      // otherwise be refused as damaged.
      const declared = Math.max(uncompressedSize, 1);
      files.push({ name, bytes: new Uint8Array(inflateRawSync(raw, { maxOutputLength: declared })) });
    } catch {
      return refuse(
        "unreadable",
...
function readShapefileUpload(bytes: Uint8Array): ReaderResult {
  const archive = readZipEntries(bytes);
  if (!archive.ok) return archive;

  const usable = archive.files.filter((file) => !isArchiveNoise(file.name));
...
type PrjResult = { ok: true; srs: ContextLayerSrs } | ContextLayerImportRefusal;
 ...
 * layer's disclosure or the map quietly ends where they were.
 */
type ShpRecord =
