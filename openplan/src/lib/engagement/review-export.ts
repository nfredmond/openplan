...
export function parseReviewSnapshot(snapshotText: string, checksum: string): EngagementReviewSnapshot {
  ...
  if (value.scope === "public" && (value.items.some((row) => row.status !== "approved" || "moderation_notes" in row || "metadata_json" in row) || value.sessions.some((row) => row.status !== "approved" || "respondent_fingerprint" in row))) throw new Error("Private records found in a public snapshot");
...
export function campaignReviewMap(snapshot: EngagementReviewSnapshot): string {
  ...
  const marks = features.map((feature) => {
    ...
    return pts.length === 1 ? `<circle cx="${pts[0][0]}" cy="${pts[0][1]}" r="4" fill="#136b73"><title>${escape(feature.id)}</title></circle>` : `<${feature.closed ? 'polygon' : 'polyline'} points="${pts.map((p)=>p.join(',')).join(' ')}" ${attrs} fill="${feature.closed ? '#136b7322' : 'none'}"><title>${escape(feature.id)}</title></${feature.closed ? 'polygon' : 'polyline'}>`;
  }).join('');
