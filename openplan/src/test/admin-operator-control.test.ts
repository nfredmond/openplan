...
describe("admin operator control summary", () => {
  it("summarizes supervised pilot readiness without implying self-serve activation", () => {
    ...
    expect(summary.preflightProofScope).toContain("Read-only pre-conversation evidence bundle");
    expect(summary.preflightProofScope).toContain("Vercel deploy-state signals");
    ...
    expect(summary.detail).toContain("internal diligence snapshot for supervised review");
    expect(summary.detail).toContain("not as a launch certificate or automated planning claim");
  ...
  it("keeps pending or unknown lanes visible as open proof gaps", () => {
    ...
    expect(summary.latestEvidenceDate).toBe("2026-05-10");
    expect(summary.detail).toContain("2 still need fresh source proof before external reliance");
