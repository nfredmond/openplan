...
vi.mock("@/lib/engagement/translation-history-server", () => ({ loadTranslationHistory: mocks.read }));
vi.mock("@/lib/observability/audit", () => ({ createApiAuditLogger: () => ({ info: vi.fn() }) }));
import { GET } from "@/app/api/engagement/campaigns/[campaignId]/translations/history/route";
const campaignId = "10000000-0000-4000-8000-000000000001";
const workspaceId = "40000000-0000-4000-8000-000000000001";
const request = () => GET(new NextRequest(`http://localhost/api/engagement/campaigns/${campaignId}/translations/history`), { params: Promise.resolve({ campaignId }) });
beforeEach(() => {
  vi.clearAllMocks();
...
it("returns private no-store history using staff write permission", async () => {
  ...
  expect(mocks.read).toHaveBeenCalledWith(expect.anything(), campaignId, workspaceId);
});
it.each([
  ["anonymous", 401, () => mocks.user.mockResolvedValue({ data: { user: null } })],
  ["viewer", 403, () => mocks.access.mockResolvedValue({ campaign: { id: campaignId, workspace_id: workspaceId }, allowed: false })],
