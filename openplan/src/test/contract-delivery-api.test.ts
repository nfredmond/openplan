...
describe("contract delivery API computation and author control",()=>{
 ...
 function context(){const f=deliveryFixture(),state={...f.state,delivery:f.delivery};mocks.rpc.mockImplementation(async(name:string)=>({data:name==="read_contract_management"?state:name==="read_contract_delivery"?f.delivery:{id:randomUUID()},error:null}));return {...f,state,command:{kind:"forecast",requestId:randomUUID(),expectedInputHash:f.delivery.inputHash,...f.options,coverageEvidence:"Synthetic source coverage",reviewEvidence:"Synthetic PM review"}};}
