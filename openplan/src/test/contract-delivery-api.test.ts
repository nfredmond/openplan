...
describe("contract delivery API computation and author control",()=>{
 ...
 function context(){const f=deliveryFixture(),state={...f.state,delivery:f.delivery};mocks.rpc.mockImplementation(async(name:string)=>({data:name==="read_contract_management"?state:name==="read_contract_delivery_version"?{inputHash:f.delivery.inputHash}:{id:randomUUID()},error:null}));return {...f,state,command:{kind:"forecast",requestId:randomUUID(),expectedInputHash:f.delivery.inputHash,...f.options,coverageEvidence:"Synthetic source coverage",reviewEvidence:"Synthetic PM review"}};}
 it("calculates server-side from unchanged input versions and strips private rates from retained inputs",async()=>{
  ...
  expect(mocks.rpc.mock.calls.slice(0,4).map(([name])=>name)).toEqual(["read_contract_management","read_contract_delivery_version","read_contract_management","read_contract_delivery_version"]);
 ...
 it("refuses a stale browser review even when server inputs remain internally consistent",async()=>{
  const f=context();const response=await saveContractCommand(request(),f.state.engagement.id,{...f.command,expectedInputHash:"0".repeat(64)});expect(response.status).toBe(400);expect((await response.json()).error).toContain("changed since you opened this page");expect(mocks.rpc.mock.calls.some(([name])=>name==="record_contract_command")).toBe(false);
 });
 it("refuses inputs that change while they are being read",async()=>{
  const f=context();let count=0;mocks.rpc.mockImplementation(async(name:string)=>({data:name==="read_contract_management"?f.state:name==="read_contract_delivery_version"?{inputHash:++count===1?f.delivery.inputHash:"changed"}:{},error:null}));
  const response=await saveContractCommand(request(),f.state.engagement.id,f.command);expect(count).toBe(2);expect(response.status).toBe(400);expect((await response.json()).error).toContain("changed during reading");expect(mocks.rpc.mock.calls.some(([name])=>name==="record_contract_command")).toBe(false);
 });
