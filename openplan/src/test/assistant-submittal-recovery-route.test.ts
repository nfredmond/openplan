...
vi.mock("@/lib/assistant/project-submittal-recovery",()=>({loadSubmittalRecoveryPage:mocks.load}));
vi.mock("@/lib/observability/audit",()=>({createApiAuditLogger:()=>({error:vi.fn()})}));
import { GET } from "@/app/api/assistant/actions/submittals/route";
...
function request(query=`workspaceId=${workspaceId}`){return new NextRequest(`http://localhost/api/assistant/actions/submittals?${query}`);}
...
describe("approved submittal read access",()=>{
