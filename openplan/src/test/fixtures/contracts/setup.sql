DO $test$ DECLARE
 ...
 p uuid:=gen_random_uuid(); element uuid:=gen_random_uuid(); ow_entry uuid:=gen_random_uuid(); ow_revision public.program_work_program_revisions; owc jsonb;
 tab text; observed integer;
...
BEGIN
 ...
 INSERT INTO public.kb_documents(id,workspace_id,uploaded_by,title,source_kind,checksum,storage_ref,status) VALUES(document,workspace,owner_id,'Synthetic agreement evidence','uploaded_pdf',repeat('a',64),'storage://kb-documents/'||workspace||'/'||document||'/synthetic.pdf','stored');
