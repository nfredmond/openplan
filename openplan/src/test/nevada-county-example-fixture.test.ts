...
describe("Nevada County evidence fixture", () => {
  it("preserves the internal-prototype validation gate and uncomfortable Max APE caveat", () => {
    ...
    const maxApe = NEVADA_COUNTY_VALIDATION_METRICS.find((metric) => metric.label === "Max APE");
    expect(maxApe?.value).toBe("237.62%");
    expect(maxApe?.note).toMatch(/disqualifies this run from outward modeling claims/i);
