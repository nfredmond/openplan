...
describe("summarizeEngagementItems", () => {
  it("counts valid route and area geometry without legacy coordinates and refuses invalid coordinates", () => {
    ...
    expect(summary.geographyCoverage).toMatchObject({ geolocatedItems: 3, nonGeolocatedItems: 4 });
    expect(summary.exportCoverage.mapReadyItems).toBe(3);
    expect(summary.sourceSummaries.find(source => source.sourceType === "public")).toMatchObject({ geolocatedCount: 3, nonGeolocatedCount: 4 });
  });
