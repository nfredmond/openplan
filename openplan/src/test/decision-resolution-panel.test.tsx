...
import { useDecisionResolution } from "@/components/engagement/decision-resolution-panel";
import { pendingDecisionResolutionKey, readDecisionResolutionRecovery } from "@/lib/engagement/decision-resolution-recovery";
...
const key = `openplan:decision-link:${scope.actorId}:${scope.workspaceId}:${scope.campaignId}:${id(4)}`;
...
function Editor({ canWrite = true, pageCopy, locked = false, editorScope = scope }: { canWrite?: boolean; pageCopy?: string; locked?: boolean; editorScope?: typeof scope }) {
  ...
  const recovery = useDecisionResolution(editorScope, { canWrite, busy, download, onResolved: resolved,
  ...
  return <><button onClick={() => recovery.begin(key, pageCopy)}>Review recovery</button>
    <button disabled={busy || recovery.blocked}>New decision link</button>{recovery.panel}</>;
}
...
const fresh = () => screen.getByRole("button", { name: "New decision link" });
...
describe("decision resolution controller", () => {
  it("retains both copies before dispatch with pinned login and archives confirmed receipts", async () => {
    ...
    fetch.mockImplementation(async (url, init) => {
      expect(String(url)).toBe(`/api/engagement/campaigns/${scope.campaignId}/decision-links/resolutions`);
      const saved = readDecisionResolutionRecovery(localStorage, scope); expect(saved.pending).toHaveLength(1);
      expect(saved.pending[0].intents).toHaveLength(2); expect(localStorage.getItem(key)).toBe(original);
      expect(init).toMatchObject({ method: "POST", cache: "no-store", headers: { "x-openplan-expected-user": scope.actorId, "x-openplan-expected-workspace": scope.workspaceId } });
    ...
    fireEvent.click(confirm()); await screen.findByText(/Recovery confirmed and copies archived/);
    expect(fetch).toHaveBeenCalledTimes(2); expect(resolved).toHaveBeenCalledOnce(); expect(fresh()).toBeEnabled();
    expect(localStorage.getItem(key)).toBeNull();
    const saved = readDecisionResolutionRecovery(localStorage, scope); expect(saved.pending).toHaveLength(0); expect(saved.archives).toHaveLength(1);
  ...
  it("does not send when proposed recovery is cancelled", () => {
    ...
    expect(fetch).not.toHaveBeenCalled(); expect(localStorage.getItem(key)).toBe(original); expect(readDecisionResolutionRecovery(localStorage, scope).pending).toHaveLength(0);
  });
  it.each(["quota", "readback"])("does not dispatch when the intent has %s storage failure", async kind => {
    ...
    vi.spyOn(Storage.prototype, "setItem").mockImplementation(function (this: Storage, name, raw) {
      if (name.startsWith("openplan:decision-resolution:")) { if (kind === "quota") throw new Error("quota"); return; } set.call(this, name, raw);
    });
  ...
  it("retries the exact resolution after acknowledgement loss and lost browser storage", async () => {
    ...
    const bundle = readDecisionResolutionRecovery(localStorage, scope).pending[0], sent = fetch.mock.calls[0][1]?.body;
    localStorage.removeItem(pendingDecisionResolutionKey(bundle)); act(() => window.dispatchEvent(new StorageEvent("storage")));
    expect(fresh()).toBeDisabled(); fireEvent.click(retry()); await screen.findByText(/Recovery confirmed/);
  ...
  it("retains both resolution identities after only the second acknowledgement is lost", async () => {
    ...
    fireEvent.click(retry()); await screen.findByText(/Recovery confirmed/);
  ...
  it.each(["bad-checksum", "different-intent", "refused"])("keeps original and pending copies on %s response", async kind => {
    ...
    expect(localStorage.getItem(key)).toBe(original); expect(readDecisionResolutionRecovery(localStorage, scope).pending).toHaveLength(1);
    expect(readDecisionResolutionRecovery(localStorage, scope).archives).toHaveLength(0); expect(resolved).not.toHaveBeenCalled(); expect(fresh()).toBeDisabled();
  });
  ...
  it.each(["unmount", "access-revoked", "workspace-changed"])("ignores late acknowledgement after %s", async kind => {
    ...
    await waitFor(() => expect(fetch).toHaveBeenCalledOnce()); const before = readDecisionResolutionRecovery(localStorage, scope).pending;
    ...
    expect(localStorage.getItem(key)).toBe(original); expect(readDecisionResolutionRecovery(localStorage, scope).pending).toEqual(before);
    expect(readDecisionResolutionRecovery(localStorage, scope).archives).toHaveLength(0); expect(resolved).not.toHaveBeenCalled();
  });
...
describe("resolution access restoration", () => {
  it.each(["response", "body"] as const)("does not revive an old %s after edit access returns", async stage => {
    ...
    const before = readDecisionResolutionRecovery(localStorage, scope).pending;
  ...
  it("retries the same resolution while the cancelled response is still pending", async () => {
    ...
    await act(async () => { second.resolve(reply(packet)); }); await screen.findByText(/Recovery confirmed and copies archived/);
    expect(resolved).toHaveBeenCalledOnce(); expect(localStorage.getItem(key)).toBeNull(); expect(readDecisionResolutionRecovery(localStorage, scope).archives).toHaveLength(1);
  });
});
