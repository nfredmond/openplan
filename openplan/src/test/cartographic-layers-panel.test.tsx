...
describe("CartographicLayersPanel", () => {
  ...
  it("renders all layer labels before counts resolve", () => {
    ...
    expect(screen.getByText("RTP corridors")).toBeInTheDocument();
    // No chips while the fetch is in flight.
    expect(document.querySelectorAll(".op-cart-layer-item__chip")).toHaveLength(0);
  });

  it("renders live counts on the three data-driven layer chips after fetch resolves", async () => {
    mockFetchJson({ projects: 1, aerial: 3, corridors: 2 });
    ...
    // RTP / engagement / transit / crashes / equity still have no data source; no chips.
    expect(chips).toHaveLength(3);
  });

  it("shows a 0 chip when the workspace has the layer but no rows", async () => {
    mockFetchJson({ projects: 0, aerial: 0, corridors: 0 });

    renderPanel();

    await waitFor(() => {
      expect(document.querySelectorAll(".op-cart-layer-item__chip").length).toBe(3);
    });

    const chips = Array.from(document.querySelectorAll(".op-cart-layer-item__chip")).map(
      (node) => node.textContent
    );
    expect(chips).toEqual(["0", "0", "0"]);
  });

  it("hides the chip for a layer whose count came back null (partial failure)", async () => {
    mockFetchJson({ projects: 1, aerial: null, corridors: 2 });

    renderPanel();

    await waitFor(() => {
      expect(document.querySelectorAll(".op-cart-layer-item__chip").length).toBe(2);
    });
  ...
  it("formats counts of 1000 or more using compact notation", async () => {
    mockFetchJson({ projects: 3800, aerial: 1, corridors: 1 });
