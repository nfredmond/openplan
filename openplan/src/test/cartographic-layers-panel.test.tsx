...
describe("CartographicLayersPanel", () => {
  ...
  it("renders live counts on the four data-driven layer chips after fetch resolves", async () => {
    mockFetchJson({ projects: 1, aerial: 3, corridors: 2, rtp: 1 });
    ...
    // Two "1" chips (projects + rtp) + one "3" + one "2" = 4 total.
    // engagement / transit / crashes / equity still have no data source; no chips.
    expect(chips).toHaveLength(4);
  });

  it("shows a 0 chip when the workspace has the layer but no rows", async () => {
    mockFetchJson({ projects: 0, aerial: 0, corridors: 0, rtp: 0 });
    ...
    expect(chips).toEqual(["0", "0", "0", "0"]);
  });

  it("hides the chip for a layer whose count came back null (partial failure)", async () => {
    mockFetchJson({ projects: 1, aerial: null, corridors: 2, rtp: 1 });
  ...
  it("formats counts of 1000 or more using compact notation", async () => {
    mockFetchJson({ projects: 3800, aerial: 1, corridors: 1, rtp: 1 });
