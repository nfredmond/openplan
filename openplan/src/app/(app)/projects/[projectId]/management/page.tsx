...
export default async function WeeklyManagement({params}:{params:Promise<{projectId:string}>}){
 ...
 return <section key={state.engagement.id} className="space-y-4 rounded border p-4"><h2 className="text-xl font-semibold"><Link className="underline" href={url}>{state.engagement.title}</Link></h2><nav className="flex flex-wrap gap-4">{["Updates","Schedule","Capacity","Forecasts","Responses"].map(section=><Link className="underline" key={section} href={`${url}?tab=remaining&section=${section}`}>{section}</Link>)}</nav>{forecast?<><p>Reviewed forecast {forecast.version}: {forecast.input_hash===state.delivery?.inputHash?"inputs unchanged":"stale; review changed inputs"}.</p><ForecastTable result={forecast.content.result}/><ContractScheduleTimeline result={forecast.content.result}/><ul className="space-y-2">{forecast.content.result.warnings.map((w,index)=><li key={index}>{w.message} <Link className="underline" href={`${url}?tab=remaining&section=${w.code.includes("capacity")||w.code.includes("availability")?"Capacity":w.code.includes("update")||w.code.includes("effort")?"Updates":"Schedule"}`}>Review affected {w.staffId?state.staff.find(s=>s.id===w.staffId)?.name??"staff reservation":w.nodeId?forecast.content.result.nodes.find(n=>n.id===w.nodeId)?.title??"schedule item":"contract inputs"}</Link></li>)}</ul></>:<p>No reviewed forecast. Review staff updates and calculate a forecast.</p>}
