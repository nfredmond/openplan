...
export default async function WorkProgramPage({ params }: { params: Promise<{ programId: string }> }) {
  ...
  try {
    ...
    for (let offset = 0; ; offset += 100) {
      const result = await supabase.from("kb_documents").select("id, title").eq("workspace_id", access.program.workspace_id).eq("source_kind", "uploaded_pdf").order("id").range(offset, offset + 99);
