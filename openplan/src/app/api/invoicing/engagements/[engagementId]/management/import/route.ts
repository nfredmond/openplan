...
export async function POST(request:NextRequest,context:{params:Promise<{engagementId:string}>}) {
 ...
 try{
  ...
  const preview=previewContractCsv(input.csv,input.filename,input.mapping,input.defaults,input.requestId,state.baselines.filter(b=>b.state==="approved").at(-1)?.content.tasks??[]);
