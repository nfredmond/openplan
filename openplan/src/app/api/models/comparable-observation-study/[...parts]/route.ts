...
import { readPublishedComparableObservationDownload } from "@/lib/models/published-comparable-observation-study";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ parts: string[] }> }) {
  const audit = createApiAuditLogger("models.comparable_observation_study.download", request);
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "Authentication required" }, { status: 401 });
  try {
    const requestedParts = (await params).parts;
    const download = await readPublishedComparableObservationDownload(requestedParts);
