...
const paramsSchema = z.object({ shareToken: z.string().regex(/^[0-9a-f]{48}$/), designationId: z.string().uuid() });
type Context = { params: Promise<{ shareToken: string; designationId: string }> };

export async function GET(request: NextRequest, context: Context) {
  const audit = createApiAuditLogger("public.land-use-plan-reviews.map", request);
  audit.info("public_land_use_plan_review_map_requested");
  ...
  const review = await loadPublicLandUsePlanReviewPacket(params.data.shareToken);
  if (!review.ok) return NextResponse.json({ error: "Map not found" }, { status: review.reason === "not_found" ? 404 : 503 });
  const map = await loadPublicDesignationMap(review.packet.content, params.data.designationId, bbox);
