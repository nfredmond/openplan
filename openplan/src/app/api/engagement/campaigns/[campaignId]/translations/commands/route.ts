...
export async function POST(request: NextRequest, context: { params: Promise<{ campaignId: string }> }) {
  ...
  try {
    ...
    const parsed = translationWriteIntentSchema.safeParse(raw);
    ...
    const written = await writeTranslations(client, { campaignId: params.data.campaignId, workspaceId: access.campaign.workspace_id }, parsed.data);
    if (written.error) {
