...
export async function GET(request: NextRequest, context: { params: Promise<{ campaignId: string }> }) {
  const audit = createApiAuditLogger("engagement.translation-history", request);
  ...
  const history = await loadTranslationHistory(client, parsed.data.campaignId, access.campaign.workspace_id);
