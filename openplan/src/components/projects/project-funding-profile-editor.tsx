    ...
    <div className="rounded-2xl border border-border/70 bg-background/80 p-4">
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <label className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Funding need</label>
          <Input value={fundingNeedAmount} onChange={(event) => setFundingNeedAmount(event.target.value)} inputMode="decimal" placeholder="2500000" />
        </div>
        <div className="space-y-1.5">
          <label className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Local match need</label>
          <Input value={localMatchNeedAmount} onChange={(event) => setLocalMatchNeedAmount(event.target.value)} inputMode="decimal" placeholder="500000" />
        </div>
      </div>

      <div className="mt-3 space-y-1.5">
        <label className="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Notes</label>
        <Textarea rows={3} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Capture the target funding need or any funding stack assumptions for this project." />
      </div>
