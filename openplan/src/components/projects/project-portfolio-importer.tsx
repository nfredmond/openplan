...
export function ProjectPortfolioImporter({
  ...
  function updateCost(index: number, patch: Partial<{ currency: string; scale: PortfolioCostScale; priceYear: number | null }>) {
    updateDraft(index, (draft) => ({
      ...draft,
      defaults: {
        ...draft.defaults,
          ...
          return <article key={sheet.index} className="space-y-3 rounded-md border p-4">
            ...
            {draft ? <div className="space-y-4 border-t pt-3">
              ...
              {draft.mapping.estimatedCost !== undefined ? <div className="grid gap-3 md:grid-cols-3">
                ...
                <label className="grid gap-1 text-xs font-semibold">Price year (optional)<input type="number" min={1800} max={3000} className="rounded border bg-background px-2 py-1.5" value={draft.defaults.cost?.priceYear ?? ""} onChange={(event) => updateCost(sheet.index, { priceYear: event.target.value === "" ? null : Number(event.target.value) })} /></label>
              </div> : null}
            </div> : null}
          </article>;
        })}
