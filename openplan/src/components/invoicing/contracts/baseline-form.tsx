...
export function BaselineForm({state,send,busy}:{state:ContractState;send:CommandSender;busy:boolean}) {
 ...
 <p className="text-sm text-muted-foreground">Save a proposal, then record its approval evidence. A proposal does not increase the approved budget. Empty amounts remain unassessed.</p>
 ...
 <p className="text-sm"><a className="underline" href={`/documents?workspaceId=${state.engagement.workspace_id}`}>Upload the agreement in Documents</a>, then reload this page. Select its retained file above.</p>
