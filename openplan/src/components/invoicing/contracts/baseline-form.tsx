...
export function BaselineForm({state,send,busy}:{state:ContractState;send:CommandSender;busy:boolean}) {
 ...
 <fieldset className="space-y-3 rounded border p-4"><legend>Authorized work period and cost basis</legend>
 <p className="text-sm">Task orders require dates within the approved master term. Funding and eligibility are documented judgments, not inferred from a selected program.</p>
 {(["startsOn","endsOn","beneficiary","funding","costBasis","eligibilityEvidence"] as const).map(k=><TextField key={k} label={{startsOn:"Authorized start",endsOn:"Authorized end",beneficiary:"Beneficiary",funding:"Funding source and agreement reference",costBasis:"Documented cost basis",eligibilityEvidence:"Eligibility review evidence"}[k]} type={k.endsWith("On")?"date":"text"} value={value.authorization?.[k]??""} onChange={v=>set("authorization",{startsOn:"",endsOn:"",beneficiary:"",funding:"",costBasis:"",eligibility:"unassessed",eligibilityEvidence:"",...value.authorization,[k]:v})}/>)}
 <Field label="Eligibility assessment"><select className={inputClass} value={value.authorization?.eligibility??"unassessed"} onChange={e=>set("authorization",{startsOn:"",endsOn:"",beneficiary:"",funding:"",costBasis:"",eligibilityEvidence:"",...value.authorization,eligibility:e.target.value as "unassessed"|"eligible"|"ineligible"})}><option value="unassessed">Unassessed</option><option value="eligible">Documented eligible</option><option value="ineligible">Documented ineligible</option></select></Field>
 </fieldset>
