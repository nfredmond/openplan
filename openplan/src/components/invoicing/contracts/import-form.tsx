...
export function ContractImportForm({state,onSaved}:{state:ContractState;onSaved:()=>Promise<void>}) {
 ...
 return <details className="space-y-4 rounded border p-4"><summary className="font-semibold">Mapped CSV intake</summary><p className="text-sm">Import up to 200 UTF-8 rows with a header. Task and staff columns accept an exact name or ID; ambiguous names are refused. The original text, file hash and row identities are retained. Existing source keys are refused, so retrying cannot duplicate costs.</p>
 ...
 <Field label="Default entry type"><select className={inputClass} value={input.defaults.category} onChange={e=>{draft.setValue({...input,defaults:{...input.defaults,category:e.target.value as ContractActual["category"]}});setPreview(null);}}><option value="labor">Labor</option><option value="expense">Expense</option></select></Field>
 ...
 <div className="flex flex-wrap gap-3"><Button type="button" disabled={busy||!input.csv} variant="outline" onClick={()=>void run(false)}>Preview mapped entries</Button><Button type="button" disabled={busy||!preview||preview.rows.some(r=>r.errors.length||!r.command)} onClick={()=>void run(true)}>Retain and import reviewed mapping</Button></div>
 ...
 {(state.imports??[]).map(i=><p key={i.id} className="text-sm break-all"><a className="underline" href={`/api/invoicing/engagements/${state.engagement.id}/management/import?importId=${i.id}`}>Download original {i.filename}</a> · SHA-256 {i.source_hash}</p>)}
 </details>;
}
