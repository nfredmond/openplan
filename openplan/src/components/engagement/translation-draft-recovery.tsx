...
import { archiveTranslationDrafts, readTranslationDraftState, retainTranslationDrafts, translationDraftKey, translationDraftStorageKey,
...
export function useTranslationDrafts(scope: TranslationDraftScope) {
  ...
  function restore() {
    try {
      const observed = readTranslationDraftState(window.sessionStorage, scope);
      const copies = [];
      ...
      storedRaw.current = observed.raw; current.current = observed.record; setRecord(observed.record);
      setArchives(copies); setReady(true); setWriteFailed(false); setMessage(null);
  ...
  function remember(next: TranslationDraftRecord) {
    ...
    try { const retained = retainTranslationDrafts(window.sessionStorage, next, storedRaw.current); storedRaw.current = JSON.stringify(retained); setWriteFailed(false); setMessage(null); }
    catch { setWriteFailed(true); setMessage("This tab could not retain the latest draft. Keep the page open and download your words before leaving or refreshing."); }
  }
  ...
  function archive() {
    try {
      archiveTranslationDrafts(window.sessionStorage, scope, current.current); restore();
    }
  ...
  const recovery = <div className="mt-3 space-y-2 text-sm">
    ...
    {writeFailed && <Button className="h-auto min-h-10 min-w-0 max-w-full whitespace-normal" type="button" variant="outline" onClick={() => { if (current.current) remember(current.current); }}>Retry retaining latest draft</Button>}
    {(message || Boolean(record?.entries.length)) && <details className="rounded-lg border border-border p-3"><summary>Unsaved translation drafts</summary>
      <p className="mt-2">These are the draft words on this page. Download them before closing the tab. If browser storage differs, preserve both copies before starting fresh.</p>
      ...
      <div className="mt-2 flex flex-wrap gap-2"><Button className="h-auto min-h-10 min-w-0 max-w-full whitespace-normal" type="button" variant="outline" onClick={() => {
        try {
          const raw = record ? JSON.stringify(record) : sessionStorage.getItem(key);
          if (raw === null) throw new Error("No stored draft copy");
          downloadDraft(raw);
        }
        catch { setMessage("The draft could not be downloaded. Keep this page open and retry recovery."); }
      }}>Download unsaved drafts</Button>
      {message && record && <Button className="h-auto min-h-10 min-w-0 max-w-full whitespace-normal" type="button" variant="outline" onClick={() => {
        try { const raw = sessionStorage.getItem(key); if (raw === null) throw new Error("No stored copy"); downloadDraft(raw); }
        catch { setMessage("The stored copy could not be downloaded. The words on this page are unchanged."); }
      }}>Download stored draft copy</Button>}
