...
import { pendingDecisionKey, readPendingDecisions, retainPendingDecision, sendPendingDecision } from "@/lib/engagement/pending-decision-link";
import type { DecisionEditorScope, PendingDecisionLink } from "@/lib/engagement/pending-decision-link";
import { useDecisionResolution } from "./decision-resolution-panel";
import { decisionResolutionHasCopy } from "@/lib/engagement/decision-resolution-recovery";
...
function DecisionEditor(props: Props) {
  ...
  const heldRequests = useRef(new Map<string, PendingDecisionLink>());
  const id = useId();
  ...
  useEffect(() => {
    let current = true, readVersion = 0;
    async function recover() {
      ...
      try {
        ...
        if (current && version === readVersion) {
          for (const row of value.pending) heldRequests.current.set(pendingDecisionKey(row), row);
          setPending([...heldRequests.current.values()]); setUnreadable(value.unreadable); setRecoveryReady(true); setStorageError(false); }
      } catch { if (current && version === readVersion) { setStorageError(true); setRecoveryReady(false); } }
    }
    void recover();
  ...
  const resolution = useDecisionResolution({ actorId, workspaceId, campaignId }, {
    canWrite: Boolean(snapshot) && !loading && !readError, busy,
    acquire() { if (locked.current) return false; locked.current = true; setBusy(true); return true; },
    release() { locked.current = false; setBusy(false); },
  ...
  const blocked = resolution.blocked || busy || loading || !snapshot || !recoveryReady || storageError || unreadable.length > 0 || pending.some(row => row.phase === "unconfirmed");
  ...
  async function retry(request: PendingDecisionLink) {
    if (locked.current || resolution.hasPending()) return;
    locked.current = true; setBusy(true);
    try {
      heldRequests.current.set(pendingDecisionKey(request), request);
      const result = await sendPendingDecision(localStorage, request);
      if (result.confirmed) heldRequests.current.delete(pendingDecisionKey(request));
  ...
  async function save(withdraw: boolean) {
    if (locked.current || blocked || resolution.hasPending() || !reason.trim() || (!withdraw && (!preview || responsesUnavailable)) || (withdraw && (!leaf || leaf.operation === "withdraw"))) return;
    locked.current = true; setBusy(true);
    try {
  ...
  return <section aria-label="Decision link editor" className="mt-3 min-w-0 space-y-4 rounded-lg border border-border p-3">
    ...
    {snapshot && !loading && unreadable.map(copy => <div key={copy.key} className="space-y-2 rounded border border-border p-2">
      ...
      <Button type="button" variant="outline" className="h-auto min-h-10 max-w-full whitespace-normal" disabled={busy} onClick={() => resolution.begin(copy.key, heldRequests.current.has(copy.key) ? JSON.stringify(heldRequests.current.get(copy.key)) : undefined)}>Review damaged request recovery</Button>
    </div>)}
    {snapshot && !loading && pending.map(request => <div key={request.intent.requestId} className="space-y-2 rounded border border-border p-2 text-sm">
      ...
      <Button type="button" variant="outline" disabled={busy || resolution.blocked} onClick={() => void retry(request)}>Retry exact request</Button>{" "}
      ...
      <Button type="button" variant="outline" className="h-auto min-h-10 max-w-full whitespace-normal" disabled={busy} onClick={() => resolution.begin(pendingDecisionKey(request), JSON.stringify(request))}>Review request recovery</Button>
    </div>)}
