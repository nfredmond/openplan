...
export function ReportingPanel({ programId, workspaceId, userId }: { programId: string; workspaceId: string; userId: string }) {
 ...
 return <div className="space-y-6 min-w-0"><nav aria-label="Work program sections" className="flex flex-wrap gap-4 text-sm underline"><a href="#actual-work">Actual work</a><a href="#period-reports">Period reports</a><a href="#budget-position">Budget position</a><a href="#reimbursement-packets">Reimbursement packets</a>{data?.canManage && <><a href="#closeout-review">Closeout review</a><a href="#closeout-reconciliation">Saved reconciliation</a></>}<a href="#preparation">Preparation and adoption history</a><a href={`/invoicing?direction=receivables&workspaceId=${encodeURIComponent(workspaceId)}#staff-time-access`}>Staff and account setup</a></nav><div className="flex flex-wrap gap-3"><Button className="h-auto min-h-10 max-w-full whitespace-normal" variant="outline" onClick={() => void reload().catch(e => setMessage(e.message))}>Reload entries</Button>{pending && <Button className="h-auto min-h-10 max-w-full whitespace-normal" disabled={busy} onClick={() => save(pending)}>Retry pending save</Button>}</div>{message && <p role="status" className="rounded-lg border p-3 text-sm">{message}</p>}{pending && <p role="alert">A save may have reached the server. Retry its retained request before creating another entry.</p>}{data ? data.revisions.length ? <><ActualEntry programId={programId} data={data} save={save} disabled={busy || !!pending}/>{data.canManage && <><PeriodReporting programId={programId} data={data} save={save} disabled={busy || !!pending}/><ReimbursementPanel programId={programId} userId={userId} reports={data.reports}/></>}</> : <p>Save a preparation revision below before attributing actual work. Adopt a baseline before issuing a management report.</p> : <p>Loading reporting entries…</p>}</div>;
}
