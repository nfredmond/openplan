...
export function WorkProgramExports({programId,revision,packet}:{programId:string;revision:number;packet?:{sequence:number;audience:"internal"|"public";publicReviewed:boolean}}) {
  const [states,setStates]=useState<Partial<Record<Format,State>>>({});
  const [error,setError]=useState<string|null>(null);
  const [downloading,setDownloading]=useState<Format|null>(null);
  const [busy,setBusy]=useState(false);
  const [generation,setGeneration]=useState(0);
  const suffix=packet?`&sequence=${packet.sequence}&audience=${packet.audience}${packet.publicReviewed?"&publicReviewed=1":""}`:"";
  const endpoint=packet?`/api/programs/${programId}/work-program/packets`:`/api/programs/${programId}/work-program/export`;
  const url=(format:Format)=>`${endpoint}?revision=${revision}&format=${format}${suffix}`;
  useEffect(()=>{
    let active=true;
    ...
    const read=async()=>{
      try {
        const results=await Promise.all(formats.map(async(format)=>{
          const response=await fetch(`${endpoint}?revision=${revision}&format=${format}${suffix}`);
  ...
  },[programId,revision,generation,endpoint,suffix]);
  async function download(format:Format) {
    const checksum=states[format]?.artifact?.checksum;
    if(!checksum)return;
    setDownloading(format);setError(null);
    try {await downloadAuthenticatedArtifact(`${url(format)}&download=1`,`work-program-revision-${revision}${packet?`-review-${packet.sequence}-${packet.audience}`:""}.${format}`,checksum);}
