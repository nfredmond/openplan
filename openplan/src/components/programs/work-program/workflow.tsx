...
export function WorkProgramWorkflow({ programId, userId, preparation, dirty, canWrite, documents, onAmendment }: Props) {
  ...
  return <section id="work-program-review" className="min-w-0 space-y-4 rounded border p-4">
    ...
    <fieldset disabled={!canWrite || !data || !revision || busy || dirty} className="min-w-0 space-y-4">
      ...
      {kind === "submit" && <><fieldset className="space-y-2"><legend>Assign independent reviewers</legend>{data?.members.filter(member => member.id !== userId && member.id !== revision?.created_by).map(member => <label key={member.id} className="flex items-start gap-2 text-sm"><input type="checkbox" checked={reviewers.includes(member.id)} onChange={event => setReviewers(current => event.target.checked ? [...current, member.id] : current.filter(id => id !== member.id))} /><span className="break-all">{member.label}</span></label>)}</fieldset><Field label="Review due date, optional" type="date" value={dueOn} onChange={setDueOn} /></>}
