...
BEGIN
  ...
  SELECT jsonb_build_object('schema',1,'campaign',jsonb_build_object(
    ...
    'questions',COALESCE((SELECT jsonb_agg(jsonb_build_object('id',q.id,'question_type',q.question_type,'prompt',q.prompt,'help_text',q.help_text,'required',q.required,'category_id',q.category_id,'config_json',q.config_json,'sort_order',q.sort_order,
      'options',COALESCE((SELECT jsonb_agg(jsonb_build_object('id',o.id,'label',o.label,'value',o.value,'sort_order',o.sort_order) ORDER BY o.sort_order,o.id) FROM engagement_survey_question_options o WHERE o.question_id=q.id AND o.is_active),'[]'::jsonb)) ORDER BY q.sort_order,q.id) FROM engagement_survey_questions q WHERE q.campaign_id=c.id AND q.is_active AND q.status='published'),'[]'::jsonb),
  ...
  ) INTO d FROM engagement_campaigns c WHERE c.id=p_campaign;
