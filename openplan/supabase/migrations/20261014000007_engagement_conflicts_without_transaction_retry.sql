-- Business conflicts must reach the reviewer instead of being retried by PostgREST.
-- Follow the existing OWP PT409 convention; PostgREST 14 retries raised 40001.
-- An incomplete receipt remains unavailable, never a successful or editable save.

CREATE OR REPLACE FUNCTION public.write_engagement_response(
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF receipt.result_json IS NULL THEN
      RAISE EXCEPTION 'Response write outcome is unavailable' USING ERRCODE = 'PT503';
    END IF;
  ...
  IF p_operation <> 'create' THEN
    ...
    IF previous.updated_at IS DISTINCT FROM p_expected_updated_at THEN
      RAISE EXCEPTION 'Response changed; review the current copy' USING ERRCODE = 'PT409';
    END IF;
...
CREATE OR REPLACE FUNCTION public.require_engagement_review_intent() RETURNS trigger LANGUAGE plpgsql SET search_path=public,pg_temp AS $$
BEGIN
