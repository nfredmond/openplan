-- Direct CSV/XLS/XLSX/ODS portfolio import with multi-sheet row identity.
-- The v0.33 function remains available during rolling deploys. Existing CSV
-- provenance is backfilled as worksheet 0 with header row 1.

ALTER TABLE public.project_portfolio_import_batches
  ADD COLUMN source_format text NOT NULL DEFAULT 'csv'
    CHECK (source_format IN ('csv','xls','xlsx','ods')),
  ADD COLUMN sheet_configurations_json jsonb;

-- Existing provenance is immutable to application code. Suspend only its
-- update/delete trigger while this migration adds the equivalent v2 shape;
-- PostgreSQL rolls the trigger state back too if the transaction fails.
ALTER TABLE public.project_portfolio_import_batches
  DISABLE TRIGGER project_portfolio_import_batches_immutable;

UPDATE public.project_portfolio_import_batches
SET sheet_configurations_json = jsonb_build_array(jsonb_build_object(
  'worksheetIndex', 0,
  'worksheetName', 'CSV',
  'headerRow', 1,
...
ALTER TABLE public.project_portfolio_import_batches
  ENABLE TRIGGER project_portfolio_import_batches_immutable;

ALTER TABLE public.project_portfolio_import_batches
  ALTER COLUMN sheet_configurations_json SET NOT NULL,
  ADD CONSTRAINT project_portfolio_import_batches_sheet_configs_check CHECK (
    jsonb_typeof(sheet_configurations_json) = 'array'
    AND jsonb_array_length(sheet_configurations_json) BETWEEN 1 AND 256
  );

CREATE OR REPLACE FUNCTION public.backfill_portfolio_csv_sheet_configuration()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public, pg_catalog
AS $$
BEGIN
  IF NEW.sheet_configurations_json IS NULL THEN
    NEW.sheet_configurations_json := jsonb_build_array(jsonb_build_object(
      'worksheetIndex', 0,
      'worksheetName', 'CSV',
      'headerRow', 1,
      'mapping', NEW.mapping_json,
      'defaults', NEW.defaults_json
    ));
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER project_portfolio_import_batches_csv_compatibility
  BEFORE INSERT ON public.project_portfolio_import_batches
  FOR EACH ROW EXECUTE FUNCTION public.backfill_portfolio_csv_sheet_configuration();

ALTER TABLE public.project_portfolio_import_rows
  ADD COLUMN source_format text NOT NULL DEFAULT 'csv'
    CHECK (source_format IN ('csv','xls','xlsx','ods')),
  ADD COLUMN worksheet_index integer NOT NULL DEFAULT 0 CHECK (worksheet_index BETWEEN 0 AND 255),
  ADD COLUMN worksheet_name text NOT NULL DEFAULT 'CSV' CHECK (char_length(worksheet_name) BETWEEN 1 AND 200),
  ADD COLUMN header_row integer NOT NULL DEFAULT 1 CHECK (header_row >= 1),
  ADD COLUMN formula_warning_fields jsonb NOT NULL DEFAULT '[]'::jsonb
    CHECK (jsonb_typeof(formula_warning_fields) = 'array');

DROP INDEX public.project_portfolio_import_rows_created_identity_uidx;
...
DROP INDEX public.project_portfolio_import_rows_batch_row_idx;
CREATE INDEX project_portfolio_import_rows_batch_row_idx
  ON public.project_portfolio_import_rows(batch_id, worksheet_index, source_row_number);
...
COMMENT ON FUNCTION public.commit_project_portfolio_import_v2(
  uuid,uuid,uuid,uuid,text,text,text,jsonb,jsonb
) IS 'Service-role-only atomic workbook import. Rechecks actor role, source scope/hash/status/format, ordered sheet metadata, row identity, formula confirmation, and cross-sheet source-ID conflicts.';
