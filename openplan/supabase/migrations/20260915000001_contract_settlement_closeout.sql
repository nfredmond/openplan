-- Immutable financial events, delivery acceptance and closeout revisions. No posted accounting entries.
CREATE TABLE public.contract_settlement_events (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),event_id uuid NOT NULL,source_key text NOT NULL,
 ...
 version integer NOT NULL CHECK(version>0),content jsonb NOT NULL,source_receipt jsonb NOT NULL,
 created_by uuid NOT NULL REFERENCES auth.users(id),created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(event_id,version)
);
CREATE INDEX contract_settlement_source_idx ON public.contract_settlement_events(engagement_id,source_key);
CREATE TABLE public.contract_deliverable_events (
 ...
 deliverable_id uuid NOT NULL REFERENCES public.project_deliverables(id),version integer NOT NULL CHECK(version>0),
 state text NOT NULL CHECK(state IN ('submitted','returned','resubmitted','accepted')),date date NOT NULL,authority text NOT NULL CHECK(length(trim(authority))>0),
 evidence text NOT NULL CHECK(length(trim(evidence))>0),source_receipt jsonb NOT NULL,
 created_by uuid NOT NULL REFERENCES auth.users(id),created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(engagement_id,deliverable_id,version)
);
CREATE TABLE public.contract_closeouts (
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),engagement_id uuid NOT NULL REFERENCES public.invoicing_engagements(id),workspace_id uuid NOT NULL REFERENCES public.workspaces(id),
 version integer NOT NULL CHECK(version>0),state text NOT NULL CHECK(state IN ('closed','reopened')),previous_id uuid REFERENCES public.contract_closeouts(id),
 input_hash text NOT NULL CHECK(input_hash ~ '^[a-f0-9]{64}$'),content jsonb NOT NULL,content_hash text NOT NULL CHECK(content_hash ~ '^[a-f0-9]{64}$'),
 created_by uuid NOT NULL REFERENCES auth.users(id),created_at timestamptz NOT NULL DEFAULT now(),UNIQUE(engagement_id,version)
);
ALTER TABLE public.contract_settlement_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.contract_deliverable_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.contract_closeouts ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.contract_settlement_events,public.contract_deliverable_events,public.contract_closeouts FROM anon,authenticated;
GRANT SELECT ON public.contract_settlement_events,public.contract_deliverable_events,public.contract_closeouts TO authenticated;
GRANT ALL ON public.contract_settlement_events,public.contract_deliverable_events,public.contract_closeouts TO service_role;
CREATE POLICY management_read ON public.contract_settlement_events FOR SELECT TO authenticated USING(public.contract_can_manage(engagement_id));
CREATE POLICY management_read ON public.contract_deliverable_events FOR SELECT TO authenticated USING(public.contract_can_manage(engagement_id));
CREATE POLICY management_read ON public.contract_closeouts FOR SELECT TO authenticated USING(public.contract_can_finance(engagement_id));
CREATE TRIGGER immutable_settlement BEFORE UPDATE OR DELETE ON public.contract_settlement_events FOR EACH ROW EXECUTE FUNCTION public.guard_contract_history();
CREATE TRIGGER immutable_deliverable_event BEFORE UPDATE OR DELETE ON public.contract_deliverable_events FOR EACH ROW EXECUTE FUNCTION public.guard_contract_history();
CREATE TRIGGER immutable_closeout BEFORE UPDATE OR DELETE ON public.contract_closeouts FOR EACH ROW EXECUTE FUNCTION public.guard_contract_history();

CREATE FUNCTION public.contract_closeout_hash(p_engagement_id uuid) RETURNS text LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,public AS $$
 SELECT encode(extensions.digest(public.contract_delivery_hash(e.workspace_id)||coalesce((SELECT string_agg(x,'|' ORDER BY x) FROM (
  SELECT 'settlement:'||id x FROM public.contract_settlement_events WHERE engagement_id=e.id
  UNION ALL SELECT 'delivery:'||id FROM public.contract_deliverable_events WHERE engagement_id=e.id
  UNION ALL SELECT 'response:'||id FROM public.contract_management_responses WHERE engagement_id=e.id
  UNION ALL SELECT 'application:'||id FROM public.contract_response_applications WHERE engagement_id=e.id
  UNION ALL SELECT 'received:'||id FROM public.contract_received_invoices WHERE engagement_id=e.id
  UNION ALL SELECT 'accounting:'||id FROM public.contract_accounting_reviews WHERE engagement_id=e.id
  UNION ALL SELECT 'record:'||public.contract_project_response_records(e.project_id)::text
 ) records),''),'sha256'),'hex') FROM public.invoicing_engagements e WHERE e.id=p_engagement_id
$$;
REVOKE ALL ON FUNCTION public.contract_closeout_hash(uuid) FROM PUBLIC,anon,authenticated,service_role;

ALTER FUNCTION public.record_contract_command(uuid,uuid,jsonb) RENAME TO record_contract_command_responses;
REVOKE ALL ON FUNCTION public.record_contract_command_responses(uuid,uuid,jsonb) FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
 ...
 IF k IN ('settlement','deliverable_event') THEN
  SELECT * INTO d FROM public.kb_documents WHERE id=coalesce((c->>'documentId')::uuid,(p_command->>'documentId')::uuid) AND workspace_id=e.workspace_id AND status='stored' AND checksum IS NOT NULL AND storage_ref IS NOT NULL AND contract_snapshot_id IS NULL AND work_program_report_id IS NULL FOR SHARE;
...
ALTER FUNCTION public.read_contract_management(uuid,uuid,timestamptz) RENAME TO read_contract_management_responses;
REVOKE ALL ON FUNCTION public.read_contract_management_responses(uuid,uuid,timestamptz) FROM PUBLIC,anon,authenticated,service_role;
...
BEGIN
 result:=public.read_contract_management_responses(p_engagement_id,p_actor_id,p_cutoff);
 IF result->>'role' IN ('owner','admin','pm','finance') THEN result:=result||jsonb_build_object('schemaVersion',5,'closeout',jsonb_build_object(
  'settlements',coalesce((SELECT jsonb_agg(to_jsonb(s) ORDER BY s.event_id,s.version) FROM public.contract_settlement_events s WHERE s.engagement_id=p_engagement_id AND s.created_at<=p_cutoff),'[]'),
  'deliverableEvents',coalesce((SELECT jsonb_agg(to_jsonb(d) ORDER BY d.deliverable_id,d.version) FROM public.contract_deliverable_events d WHERE d.engagement_id=p_engagement_id AND d.created_at<=p_cutoff),'[]'),
  'versions',coalesce((SELECT jsonb_agg(CASE WHEN result->>'role'='pm' THEN to_jsonb(c)||jsonb_build_object('content',c.content-'package') ELSE to_jsonb(c) END ORDER BY c.version) FROM public.contract_closeouts c WHERE c.engagement_id=p_engagement_id AND c.created_at<=p_cutoff),'[]'),
  'inputHash',public.contract_closeout_hash(p_engagement_id)));
 END IF;
 RETURN result;
END $$;
...
BEGIN
 IF EXISTS(SELECT 1 FROM (SELECT b.source_receipts FROM public.contract_baselines b UNION ALL SELECT jsonb_build_array(m.source_receipt) FROM public.contract_master_terms m UNION ALL SELECT jsonb_build_array(s.source_receipt) FROM public.contract_settlement_events s UNION ALL SELECT jsonb_build_array(d.source_receipt) FROM public.contract_deliverable_events d) retained CROSS JOIN LATERAL jsonb_array_elements(retained.source_receipts) r WHERE r->>'id'=OLD.id::text) THEN
...
BEGIN
 IF EXISTS(SELECT 1 FROM (SELECT b.source_receipts FROM public.contract_baselines b UNION ALL SELECT jsonb_build_array(m.source_receipt) FROM public.contract_master_terms m UNION ALL SELECT jsonb_build_array(s.source_receipt) FROM public.contract_settlement_events s UNION ALL SELECT jsonb_build_array(d.source_receipt) FROM public.contract_deliverable_events d) retained CROSS JOIN LATERAL jsonb_array_elements(retained.source_receipts) r WHERE
