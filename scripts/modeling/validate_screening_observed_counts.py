...
# The worker owns the shared count-validation rules: two lanes computing "is
# this a divided highway" or "do these stations agree" from the same data by
# different rules is how the app and a report come to disagree about one road.
# `resolve_shared_links` lived here alone until 2026-08-18, and the worker — the
# lane every planner-visible figure comes from — graded a shared link once per
...
def resolve_shared_links(results: list[dict[str, Any]], *, consistency_ratio: float | None = None) -> dict[str, Any]:
    """This lane's rows name the link `model_link_id`; the worker's name it `link_id`.

    The rules themselves are the worker's, imported rather than reimplemented.
    """
    return _worker_resolve_shared_links(
        results, consistency_ratio=consistency_ratio, link_id_field="model_link_id"
    )
