...
`20261014000013_engagement_translation_generation.sql` and
`20261014000014_engagement_translation_generation_reads.sql`. They add retained command
receipts, complete source/current-version snapshots, history receipts and private
generation attempts/output with a staff-only reader. The queue API retains exact
...
For new model calls, the translation helper preserves supported source text and
refuses incomplete model output. Previously cached translations lack completion
provenance and have not been revalidated by this change. The existing output-token
ceiling remains; long translations may be unavailable rather than being presented
as complete. Atomic translation writes,
source-text custody and durable generation remain in development. Earlier source
words and correction reasons remain unknown unless retained separately.
Publication and final CI are recorded in the translation-custody review.
...
Desktop and 390px journeys through the actual native executable cover connection
files, frozen answers, exact retries, approved drafts, cancellation, revocation
and retained history. Native tests use synthetic credentials and local scripted
...
- `20260911000003_contract_documents.sql`: `contract_documents`
- `20260911000004_contract_imports.sql`: `contract_imports`
...
- Engagement requires these additive migrations before application and worker updates:
  ...
  [engagement_campaign_configuration_scope](openplan/supabase/migrations/20260908000011_engagement_campaign_configuration_scope.sql).
...
- Apply these additive migrations before the updated application/worker:
  [document_extraction_recovery](openplan/supabase/migrations/20260907000001_document_extraction_recovery.sql),
  [work_program_extraction_versions](openplan/supabase/migrations/20260907000002_work_program_extraction_versions.sql),
  [work_program_preparation_references](openplan/supabase/migrations/20260907000003_work_program_preparation_references.sql),
  [work_program_export_custody](openplan/supabase/migrations/20260907000004_work_program_export_custody.sql),
  [work_program_recovery_forward_repair](openplan/supabase/migrations/20260907000005_work_program_recovery_forward_repair.sql), and
  [document_extraction_cancel](openplan/supabase/migrations/20260907000006_document_extraction_cancel.sql).
