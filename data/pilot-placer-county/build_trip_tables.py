#!/usr/bin/env python3
"""
Build trip generation and OD tables for Placer County pilot from LODES/Census data.
...
# Filter to Placer County tracts
wac_nev = wac[wac["tract_geoid"].isin(zone_geoids)].copy()
print(f"  Placer County blocks: {len(wac_nev)}")
...
for chunk in pd.read_csv(os.path.join(DATA_DIR, "ca_od_main_JT00_2021.csv.gz"), chunksize=500000):
    ...
    # Keep flows where at least one end is in Placer County
...
print(f"  Flows involving Placer County: {len(od_nev):,}")
...
# Placer County actual pop ~415,000 per Census. Adjust if needed.
actual_pop = 415000
...
print(f"\n  Estimated population: {emp_by_tract['est_population'].sum():,} (scaled to ~415k)")
